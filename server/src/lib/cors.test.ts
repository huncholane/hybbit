...
describe("CORS policy", () => {
  it("allows credentialed management requests only from BASE_URL origin", async () => {
    const app = await buildCorsTestApp({
      NODE_ENV: "production",
      BASE_URL: "https://hygo.example.com/dashboard",
    });
    ...
    try {
      const res = await app.inject({
        ...
        url: "/api/sites/1",
        headers: {
          origin: "https://hygo.example.com",
          "access-control-request-method": "DELETE",
        },
      });

      expect(res.statusCode).toBe(204);
      expect(res.headers["access-control-allow-origin"]).toBe("https://hygo.example.com");
  ...
  it("does not emit CORS headers for disallowed management origins", async () => {
    const app = await buildCorsTestApp({
      NODE_ENV: "production",
      BASE_URL: "https://hygo.example.com",
    });
  ...
  it("treats /api/mcp like other authenticated routes: trusted origins or no Origin at all", async () => {
    ...
    try {
      ...
      expect(preflight.statusCode).toBe(204);
      expect(preflight.headers["access-control-allow-origin"]).toBe("https://hygo.example.com");
  ...
  it("allows public session-list requests without credentials", async () => {
    ...
    try {
      const res = await app.inject({
        ...
        headers: {
          origin: "https://docs.hygo.ai",
        },
      });

      expect(res.statusCode).toBe(200);
      expect(res.headers["access-control-allow-origin"]).toBe("https://docs.hygo.ai");
      expect(res.headers["access-control-allow-credentials"]).toBeUndefined();
    } finally {
      await app.close();
    }
  ...
  it("allows credentialed session-list requests from the dashboard origin", async () => {
    ...
    try {
      const res = await app.inject({
        ...
        url: "/api/sites/81/sessions?past_minutes_start=240&past_minutes_end=0&filters=[]&page=1&limit=100",
        headers: {
          origin: "https://demo.hygo.ai",
        },
      });

      expect(res.statusCode).toBe(200);
      expect(res.headers["access-control-allow-origin"]).toBe("https://demo.hygo.ai");
      expect(res.headers["access-control-allow-credentials"]).toBe("true");
    } finally {
      await app.close();
    }
...
describe("trusted CORS origins", () => {
  it("normalizes quoted URLs to origins", () => {
    expect(normalizeCorsOrigin('"https://hygo.example.com/path"')).toBe("https://hygo.example.com");
  ...
  it("uses BASE_URL in production and localhost client origins in development", () => {
    ...
    ).toEqual(["https://hygo.example.com"]);
    ...
    ).toEqual(["https://hygo.example.com", "http://localhost:3002", "http://127.0.0.1:3002"]);
  });
