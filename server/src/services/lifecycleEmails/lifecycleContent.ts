...
const appUrl = () => (process.env.BASE_URL || "https://app.hygo.ai").replace(/\/$/, "");
...
export const noSite1 = (name?: string | null): LifecycleEmail => ({
  subject: "Finish setting up Hygo",
  text: `${greeting(name)}
...
You signed up for Hygo a few days ago but haven't added a site. That usually means something got in the way - a question about pricing, self-hosting, privacy, or just timing.

Whatever it was, I'd genuinely like to know. Reply with one line and I'll help, or tell me what we could have done better.

This is the last email I'll send unless you start using Hygo.${signoff}`,
});
...
Your Hygo snippet hasn't sent any data from ${domain} yet. The three most common causes:
...
${domain} still isn't sending data to Hygo. I won't keep nudging you - this is the last one.

If you hit a technical wall, reply with your platform (WordPress, Next.js, Shopify...) and I'll send you exact instructions. If you decided Hygo isn't for you, I'd honestly love to know why.
...
export const siteLive = (domain: string, siteId: number, country: string | null, name?: string | null): LifecycleEmail => ({
  subject: `Hygo is live on ${domain}`,
  text: `${greeting(name)}

Hygo just received its first pageview from ${domain}${country ? ` - a visitor from ${country}` : ""}. Everything is working.
...
${domain} has been on Hygo for a few days. So far:
...
One thing worth knowing: adblockers hide roughly 10-30% of real visitors from Google Analytics, but they rarely block Hygo. If these numbers look higher than what you're used to, that's your actual traffic.
...
window.hygo.event("signup_clicked")

Or straight from HTML, no code:

<button data-hygo-event="signup_clicked">Sign up</button>

Events show up on your dashboard and work as funnel steps and goals.

Guide: https://hygo.ai/docs/track-events${signoff}`,
});
...
Set a goal on that path and Hygo shows your conversion rate broken down by source, country, and device. Funnels go further: define the steps leading up to it and see exactly where people drop off.

Create your first goal: ${appUrl()}/${siteId}/goals

Guide: https://hygo.ai/docs/goals${signoff}`,
});
...
${domain} was sending data to Hygo, but we haven't received anything since ${lastEventAt}. If you redeployed recently, the tracking snippet may have been dropped from the new build - that's the usual cause.
