...
const GUIDE_BASE = "https://hygo.ai/docs/guides";
    ...
    for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
      ...
      try {
        response = await fetch(url, {
          ...
          headers: {
            "User-Agent": "Mozilla/5.0 (compatible; HygoSetupCheck/1.0; +https://hygo.ai)",
            Accept: "text/html",
          },
        });
      } catch (error) {
...
/**
 * Whether the fetched homepage loads the Hygo tracking script *for this
 * site*: a script reference plus a matching data-site-id, so a leftover
 * snippet for a different site doesn't report success.
 */
export function hasHygoScript(html: string, siteId: number): boolean {
  if (!/api\/script\.js|hygo\.js/i.test(html)) return false;
