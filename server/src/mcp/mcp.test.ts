...
describe("mcp endpoint", () => {
  ...
  it("advertises RFC 9728 resource metadata on 401 when BASE_URL is set", async () => {
    ...
    process.env.BASE_URL = "https://hygo.example.com";
    try {
      const response = await app.inject({
        method: "POST",
        url: "/api/mcp",
      ...
      expect(response.headers["www-authenticate"]).toContain(
        'resource_metadata="https://hygo.example.com/.well-known/oauth-protected-resource"'
      );
  ...
  it("responds to initialize and keeps responses out of caches", async () => {
    ...
    expect(result.serverInfo.name).toBe("hygo");
