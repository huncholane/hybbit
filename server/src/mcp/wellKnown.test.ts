...
const AS_METADATA = {
  issuer: "https://hygo.example.com",
  authorization_endpoint: "https://hygo.example.com/api/auth/mcp/authorize",
  token_endpoint: "https://hygo.example.com/api/auth/mcp/token",
  registration_endpoint: "https://hygo.example.com/api/auth/mcp/register",
};

const PR_METADATA = {
  resource: "https://hygo.example.com/api/mcp",
  authorization_servers: ["https://hygo.example.com"],
...
describe("getResourceMetadataUrl", () => {
  it("builds the metadata URL from BASE_URL, tolerating trailing slashes", () => {
    expect(getResourceMetadataUrl({ BASE_URL: "https://hygo.example.com" } as NodeJS.ProcessEnv)).toBe(
      "https://hygo.example.com/.well-known/oauth-protected-resource"
    );
    expect(getResourceMetadataUrl({ BASE_URL: "https://hygo.example.com/" } as NodeJS.ProcessEnv)).toBe(
      "https://hygo.example.com/.well-known/oauth-protected-resource"
    );
