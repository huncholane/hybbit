...
import { HygoApiError } from "../apiClient.js";
...
export function createGuard(log?: (message: string) => void): ToolGuard {
  return handler => async args => {
    try {
      ...
      if (error instanceof HygoApiError) {
        const hint = ERROR_HINTS[error.status];
        return fail(`Hygo API error ${error.status}: ${error.message}${hint ? ` — ${hint}` : ""}`);
      }
      ...
      return fail("Hygo could not complete the request");
    }
  };
}
