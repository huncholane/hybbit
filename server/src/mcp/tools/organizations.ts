...
export function registerOrganizationTools(server: McpServer, api: HygoApiClient, guard: ToolGuard, allowed: ScopeCheck): void {
  if (allowed("org", "read"))
  server.registerTool(
    "list_members",
    {
  ...
  server.registerTool(
    "add_member",
    {
      ...
      description:
        "Add an existing Hygo user to the organization by email. The user must already have a Hygo account. Requires an org admin/owner key; only an owner key can grant the owner role.",
      inputSchema: {
        organization_id: organizationIdInput,
        email: z.string().email().describe("Email of an existing Hygo user"),
