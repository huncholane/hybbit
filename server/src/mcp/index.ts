...
import { HygoApiClient } from "./apiClient.js";
...
const INSTRUCTIONS = `Hygo web analytics: read tools for traffic and behavior data, plus write tools to manage sites, goals, funnels, organization members, teams, and user profiles.
...
function buildMcpServer(
  ...
  const server = new McpServer({ name: "hygo", version: "0.3.0" }, { instructions: INSTRUCTIONS });
  registerTools(server, new HygoApiClient(fastify, authorization, handoffNonce), config);
  return server;
}
      ...
      if (error instanceof McpAuthenticationError) {
        if (error.statusCode === 401) {
          ...
          reply.header(
            "WWW-Authenticate",
            resourceMetadataUrl
              ? `Bearer realm="hygo-mcp", resource_metadata="${resourceMetadataUrl}"`
              : 'Bearer realm="hygo-mcp"'
          );
