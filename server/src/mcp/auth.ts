 ...
 * protocol message is processed: first as a Hygo API key, then as an OAuth
  ...
  return async (request: FastifyRequest): Promise<McpAuthContext> => {
    ...
    if (!bearerToken) {
      throw new McpAuthenticationError(
        "Unauthorized: send a Hygo API key as 'Authorization: Bearer <key>' (Settings > Account > Personal API Keys), or connect with an OAuth-capable MCP client.",
        401
      );
    }
    ...
    switch (identity.status) {
      ...
      case "verify_error":
        throw new McpAuthenticationError("Hygo could not verify the API key", 503);
      default:
