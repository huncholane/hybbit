...
describe("sanitizeClickhouseError", () => {
  ...
  it("maps privilege errors to a generic message", () => {
    expect(
      sanitizeClickhouseError(new Error("Code: 497. DB::Exception: hygo_query: Not enough privileges. (ACCESS_DENIED)"))
  ...
  it("collapses non-user-facing error classes to the code only", () => {
    expect(
      sanitizeClickhouseError(
        new Error("Code: 1. DB::Exception: user hygo_query failed reading /var/lib/clickhouse/store/x.bin on host ch-1")
      )
