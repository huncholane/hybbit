...
describe("buildQueryUserStatements", () => {
  it("escapes the password and pins every limit as READONLY", () => {
    const statements = buildQueryUserStatements("analytics", "hygo_query", "a&b<c'\"\\d");
    expect(statements.find(s => s.startsWith("CREATE USER"))).toBe(
      "CREATE USER IF NOT EXISTS hygo_query IDENTIFIED WITH sha256_password BY 'a&b<c\\'\\\"\\\\d'"
    );
    ...
    expect(statements[statements.length - 2]).toBe("REVOKE ALL ON *.* FROM hygo_query");
    expect(statements[statements.length - 1]).toBe("GRANT SELECT ON analytics.events TO hygo_query");
  });

  it("rejects non-identifier database or user names", () => {
    expect(() => buildQueryUserStatements("analytics; DROP", "hygo_query", "x")).toThrow();
