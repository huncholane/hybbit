---
title: Track Events
description: How to track custom events with Hygo
---

The script exposes a global object `window.hygo` with functions for manual tracking of events, as well as the ability to track events using data attributes on HTML elements.
...
<button
  data-hygo-event="purchase_click"
  data-hygo-prop-product="premium"
  data-hygo-prop-price="49.99"
>
  Buy Now
</button>
```
...
### `window.hygo.event(eventName, properties)`
...
window.hygo.event("Signup Button Clicked");

// Track a custom event with properties
window.hygo.event("Item Added To Cart", {
  itemId: "PROD123",
  price: 49.99,
  category: "Clothing"
});
...
Put this as `hygo.d.ts` anywhere in your project to avoid having to do `(window as any).hygo.event()` everywhere.
