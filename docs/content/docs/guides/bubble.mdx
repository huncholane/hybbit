---
title: Bubble.io
description: Integrate Hygo Analytics with your Bubble.io application to track user behavior
---
...
This guide explains how to integrate Hygo with your Bubble.io application to track user behavior, page views, and custom events.

## How to Add Hygo to Your Bubble App
...
Navigate to your Hygo dashboard to obtain your code snippet. It will look like this:
...
-   Paste your Hygo tracking snippet into this box. This will add the script to the `<head>` section of every page in your Bubble app.
...
Open your live Bubble application and navigate through a few pages. Then, check your Hygo dashboard to see if data is being received. You can also inspect your browser's network tab to confirm that `script.js` is loaded.
</Step>
</Steps>

<Callout type="info">
  **Page View Tracking in Bubble Apps**: Bubble applications often behave like Single Page Applications (SPAs) for navigation between "pages" (which are often groups shown/hidden on a single actual HTML page). Hygo's script is designed to detect URL changes in SPAs.
  - If your Bubble app uses actual URL route changes for different views, Hygo should track these automatically.
  - If navigation primarily involves showing/hiding groups without URL changes, Hygo will only track the initial page load. For more granular tracking of "virtual" page views in such cases, you would need to use custom event tracking.
  - If URL changes occur but are not automatically tracked, you might need to trigger `window.hygo.pageview()` manually using a Bubble workflow action that runs JavaScript when a "page" (group) becomes visible.
</Callout>
...
<Step>
### Add Hygo Tracking Code

-   In the "Run JavaScript" action's code block, add your Hygo event tracking code:

```javascript
if (typeof window.hygo !== 'undefined') {
  window.hygo.event('bubble_event_name', {
...
if (typeof window.hygo !== 'undefined') {
  window.hygo.event('item_interaction', {
...
-   Check your Hygo dashboard to see if the custom event data is being received.
</Step>
</Steps>

By adding the global script and using "Run JavaScript" workflow actions, you can effectively integrate Hygo analytics into your Bubble.io application.