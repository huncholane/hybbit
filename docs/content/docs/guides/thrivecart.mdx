---
title: "ThriveCart"
description: "Integrate Hygo Analytics with your ThriveCart checkout pages"
---
...
Hygo can be integrated with ThriveCart to track cart views, purchases, and other conversion events on your checkout pages.

<Callout type="warn">
  ThriveCart strips out the standard `data-site-id` attribute from scripts. You must use the alternative installation method below for Hygo to work correctly.
</Callout>

## Adding the Tracking Script

The standard Hygo script won't work in ThriveCart because ThriveCart removes the `data-site-id` attribute. Instead, use this JavaScript snippet that dynamically creates the script element:
...
(function() {
  ...
  s.src = 'https://app.hygo.ai/api/script.js';
  s.async = true;
...
Replace `YOUR_SITE_ID` with your actual Hygo Site ID from your dashboard.
</Step>

<Step>
### Save and Test

Save your changes and visit your checkout page. Check your Hygo dashboard to confirm pageviews are being tracked.
</Step>
</Steps>
...
(function () {
  ...
  var SENT_KEY = "hygo_" + EVENT_NAME + "_sent_v1";
  try {
  ...
  function isReady() {
    return window.hygo && typeof window.hygo.event === "function";
  }

  function send() {
    window.hygo.event(EVENT_NAME, PAYLOAD);
  }
  ...
  function ensureScriptLoaded() {
    ...
    if (document.querySelector('script[src="https://app.hygo.ai/api/script.js"]')) return;
  ...
  // If Hygo is already loaded, send immediately
  if (isReady()) return send();
...
If your ThriveCart is hosted on a subdomain (e.g., `sales.yourdomain.com`) while your main site is on `yourdomain.com`, Hygo will track both as part of the same site as long as you use the same Site ID. This allows you to create funnels that track the user journey from your main site through to the ThriveCart checkout and purchase.

## Troubleshooting

### Events not appearing in Hygo

1. **Check the script is loading**: Open your browser's Developer Tools (F12), go to the Network tab, and look for `script.js` being loaded from `app.hygo.ai`.
2. **Check for JavaScript errors**: In the Console tab, look for any red error messages.
3. **Verify your Site ID**: Make sure you've replaced `YOUR_SITE_ID` with your actual Site ID from the Hygo dashboard.
