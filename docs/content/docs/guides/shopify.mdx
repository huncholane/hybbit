...
<script type="text/javascript">
  if (typeof window.hygo !== 'undefined' && {{ checkout | json }}) {
    const hygoCheckout = {{ checkout | json }};
    let items = [];
    hygoCheckout.line_items.forEach(function(item) {
      items.push({
        item_id: item.sku || item.variant_id,
    ...
    window.hygo.event('purchase', {
      transaction_id: hygoCheckout.order_number,
      value: parseFloat(hygoCheckout.total_price),
      tax: parseFloat(hygoCheckout.tax_price),
      shipping: parseFloat(hygoCheckout.shipping_price),
      currency: hygoCheckout.currency,
      items: items
    });
  }
</script>
