...
Hygo doesn't push a websocket stream, but the events endpoint is built for polling: pass a `since_timestamp` and you get only the events that happened after it. This guide builds a self-advancing feed loop you can wire into a dashboard, a terminal ticker, or an alerting rule.
...
const API = 'https://app.hygo.ai';
const SITE = '123';
const API_KEY = process.env.HYGO_API_KEY;
...
API = "https://app.hygo.ai"
SITE = "123"
API_KEY = os.environ["HYGO_API_KEY"]
POLL_SECONDS = 10
