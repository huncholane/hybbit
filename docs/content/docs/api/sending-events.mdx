...
1. Navigate to **Settings → Account** in your Hygo dashboard
...
<Tab value="Pageview">
```bash
curl -X POST 'https://app.hygo.ai/api/track' \
...
const trackEvent = async (eventData) => {
  const response = await fetch('https://app.hygo.ai/api/track', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${process.env.HYGO_API_KEY}`,
    },
    body: JSON.stringify({
      site_id: process.env.HYGO_SITE_ID,
      ...eventData
    })
  });
...
def track_event(event_data):
    response = requests.post(
        'https://app.hygo.ai/api/track',
        headers={
            'Content-Type': 'application/json',
            'Authorization': f"Bearer {os.getenv('HYGO_API_KEY')}"
        },
        json={
            'site_id': os.getenv('HYGO_SITE_ID'),
            **event_data
        }
    )
    return response.json()
...
func trackEvent(event TrackingEvent) error {
    event.SiteID = os.Getenv("HYGO_SITE_ID")
    ...
    req.Header.Set("Authorization", "Bearer "+os.Getenv("HYGO_API_KEY"))
...
public class HygoTracker {
    private static final String API_URL = "https://app.hygo.ai/api/track";
    private static final String API_KEY = System.getenv("HYGO_API_KEY");
    private static final String SITE_ID = System.getenv("HYGO_SITE_ID");
    ...
    public static void main(String[] args) throws Exception {
        HygoTracker tracker = new HygoTracker();
...
class HygoTracker {
    private $apiUrl = 'https://app.hygo.ai/api/track';
    private $apiKey;
    private $siteId;

    public function __construct() {
        $this->apiKey = getenv('HYGO_API_KEY');
        $this->siteId = getenv('HYGO_SITE_ID');
    }
...
class HygoTracker
  API_URL = 'https://app.hygo.ai/api/track'

  def initialize
    @api_key = ENV['HYGO_API_KEY']
    @site_id = ENV['HYGO_SITE_ID']
  end
...
public class HygoTracker
{
    ...
    private const string ApiUrl = "https://app.hygo.ai/api/track";

    public HygoTracker()
    {
        _httpClient = new HttpClient();
        _apiKey = Environment.GetEnvironmentVariable("HYGO_API_KEY");
        _siteId = Environment.GetEnvironmentVariable("HYGO_SITE_ID");
...
impl HygoTracker {
    fn new() -> Self {
        Self {
            ...
            api_key: env::var("HYGO_API_KEY").expect("HYGO_API_KEY not set"),
            site_id: env::var("HYGO_SITE_ID").expect("HYGO_SITE_ID not set"),
            api_url: "https://app.hygo.ai/api/track".to_string(),
        }
    }
...
class HygoTracker {
    static let shared = HygoTracker()

    private let apiUrl = "https://app.hygo.ai/api/track"
    private let apiKey: String
    private let siteId: String

    init() {
        // Ensure API key and site ID are available
        guard let apiKey = ProcessInfo.processInfo.environment["HYGO_API_KEY"],
              !apiKey.isEmpty,
              let siteId = ProcessInfo.processInfo.environment["HYGO_SITE_ID"],
              !siteId.isEmpty else {
            fatalError("HYGO_API_KEY and HYGO_SITE_ID environment variables must be set")
        }
        self.apiKey = apiKey
        self.siteId = siteId
    }
    ...
    func trackEvent(_ eventData: [String: Any]) async throws {
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw NSError(domain: "HygoTracker", code: 1, userInfo: [NSLocalizedDescriptionKey: "Tracking failed"])
        }
    }
...
Task {
    // Track a pageview
    try await HygoTracker.shared.trackEvent([
        "type": "pageview",
        "pathname": "/api/users",
        "hostname": "api.example.com",
        "page_title": "User API Endpoint"
...
class HygoTracker(private val context: Context) {
    companion object {
        private const val API_URL = "https://app.hygo.ai/api/track"
        private val JSON = "application/json; charset=utf-8".toMediaType()
    }

    private val client = OkHttpClient()
    private val apiKey = BuildConfig.HYGO_API_KEY // Set in build.gradle
    private val siteId = BuildConfig.HYGO_SITE_ID
...
class MainActivity : AppCompatActivity() {
    private val tracker = HygoTracker(this)
...
class HygoTracker {
  static final HygoTracker _instance = HygoTracker._internal();
  factory HygoTracker() => _instance;
  HygoTracker._internal();

  static const String _apiUrl = 'https://app.hygo.ai/api/track';
  static const String _apiKey = String.fromEnvironment('HYGO_API_KEY');
  static const String _siteId = String.fromEnvironment('HYGO_SITE_ID');
...
<Callout type="info">
**Browser Tracking**: For standard browser-based tracking, use the [Hygo Web SDK](/docs/sdks/web) instead. The HTTP API is designed for server-side applications, mobile apps, and custom integrations.
</Callout>
