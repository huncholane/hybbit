...
description: Set up Hygo with your own Nginx installation instead of using the built-in Caddy server
---
...
This guide shows you how to set up Hygo with your own Nginx installation instead of using the built-in Caddy server.
...
- Hygo running with `--no-webserver` flag

## Setup Steps

<Steps>
<Step>
### Run Hygo without Caddy

First, set up Hygo to expose ports without the built-in webserver:
...
If you need to use custom ports for Hygo:
...
**502 Bad Gateway**: Check if Hygo services are running:
```bash
docker compose ps
docker compose logs
```
