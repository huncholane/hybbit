...
description: Manual setup guide for Hygo using Docker Compose without the setup script
---
...
This guide is for users who want to manually set up Hygo using Docker Compose without the setup script. This is useful if you already have a reverse proxy setup (like Nginx, Traefik, or Coolify), or if you want more control over the configuration.
...
```bash
git clone https://github.com/hygo-ai/hygo.git
cd hygo
```
</Step>
...
services:
  backend:
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.hygo-api.rule=Host(`your.domain.com`) && PathPrefix(`/api`)"
      - "traefik.http.routers.hygo-api.tls.certresolver=letsencrypt"
      - "traefik.http.services.hygo-api.loadbalancer.server.port=3001"
  
  client:
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.hygo-client.rule=Host(`your.domain.com`)"
      - "traefik.http.routers.hygo-client.tls.certresolver=letsencrypt"
      - "traefik.http.services.hygo-client.loadbalancer.server.port=3002"
```
</Tab>
</Tabs>
</Step>
...
Hygo consists of these services:
...
services:
  ...
  backend:
    image: ghcr.io/hygo-ai/hygo-backend:${IMAGE_TAG:-latest}
    ...
    labels:
      ...
      - traefik.http.routers.hygo-backend.entrypoints=https
      - traefik.http.routers.hygo-backend.tls=true
      - traefik.http.routers.hygo-backend.rule=Host(`${DOMAIN_NAME}`) && (Path(`/api`) || PathPrefix(`/api/`))
      - traefik.http.services.hygo-backend.loadbalancer.server.port=3001
      - traefik.http.routers.hygo-backend.priority=100
      - traefik.http.middlewares.hygo-forward-headers.headers.customrequestheaders.X-Forwarded-Proto=https
      - traefik.http.middlewares.hygo-forward-headers.headers.customrequestheaders.X-Forwarded-Host=${DOMAIN_NAME}
      - traefik.http.routers.hygo-backend.middlewares=hygo-forward-headers

  client:
    image: ghcr.io/hygo-ai/hygo-client:${IMAGE_TAG:-latest}
    ...
    labels:
      - traefik.enable=true
      - traefik.docker.network=coolify
      - traefik.http.routers.hygo-client.entrypoints=https
      - traefik.http.routers.hygo-client.tls=true
      - traefik.http.routers.hygo-client.rule=Host(`${DOMAIN_NAME}`)
      - traefik.http.services.hygo-client.loadbalancer.server.port=3002
      - traefik.http.routers.hygo-client.priority=10
...
To deploy Hygo on Coolify, you need to:
...
- `hygo_clickhouse-data`: Analytics data
- `hygo_postgres-data`: User accounts and site configurations
...
These guides provide step-by-step instructions for integrating Hygo with your existing infrastructure.
