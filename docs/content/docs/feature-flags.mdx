---
title: Feature Flags
description: Roll out features, serve remote config, and target variants in Hygo
---
...
Feature flags let you change product behavior without deploying new code. In Hygo, a flag can be evaluated in the browser, on your server, or in both places. Flag assignments are also attached to analytics events so you can filter and compare behavior by flag value.
...
Boolean flags support a rollout percentage for each condition set. If the visitor matches the condition set and falls inside the rollout bucket, `window.hygo.flag()` returns `true`.

```javascript
window.hygo.onReady((hygo) => {
  const enabled = hygo.flag("new_checkout", false);
...
```javascript
window.hygo.onReady((hygo) => {
  const variant = hygo.flag("homepage_cta", "control");
...
Remote config flags are useful when you want Hygo to choose a JSON payload. For remote config, `flag()` returns whether a config matched, and `flagPayload()` returns the actual config.

```javascript
window.hygo.onReady((hygo) => {
  const config = hygo.flagPayload("checkout_config", {
    buttonText: "Continue",
    theme: "default",
  });
...
1. Hygo checks condition sets from top to bottom.
2. The first condition set whose rules all match is selected.
3. Hygo applies the rollout, variant allocation, or remote config payload from that selected set.
...
<Callout type="warn">
If a visitor matches a boolean condition set but falls outside that set's rollout percentage, Hygo returns `false` and does not continue to later condition sets.
</Callout>
...
window.hygo.onReady((hygo) => {
  const enabled = hygo.flag("new_checkout", false);
  const payload = hygo.flagPayload("new_checkout", {});
});
```
...
<Tabs items={['cURL', 'JavaScript']}>
  <Tab value="cURL">
```bash
curl -X POST "https://app.hygo.ai/api/sites/123/feature-flags/evaluate" \
...
const response = await fetch(
  "https://app.hygo.ai/api/sites/123/feature-flags/evaluate",
  {
    method: "POST",
    headers: {
