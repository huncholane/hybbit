---
title: MCP
description: Let an AI assistant explore and manage your Hygo analytics through the hosted Model Context Protocol server
---
...
1. Open **Settings → Account** in Hygo.
2. Create a key in the **Personal API Keys** section.
3. Copy it immediately. Hygo only shows a new key once.
...
Use this Streamable HTTP URL for Hygo Cloud:

```text
https://app.hygo.ai/api/mcp
```
...
> List my Hygo sites, summarize last month's traffic for the docs site, and create a goal for signups.
...
| `add_member`                | Add an existing Hygo user to the organization *(admin)*        |
...
OAuth clients request scopes through the standard grant. Hygo advertises the same `resource:action` scopes in its discovery metadata; a grant carrying no custom scopes (or only the standard OIDC scopes) is treated as full access, for backward compatibility.
...
- The server does not contain an AI model or send analytics to an AI provider. Data only leaves Hygo in response to the MCP client you configure.
...
- **503 Service Unavailable:** Hygo could not verify the key. Retry the request; do not replace a known-valid key based on this response.
