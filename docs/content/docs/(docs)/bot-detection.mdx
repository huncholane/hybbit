...
Hygo still stores a compact bot event record so you can inspect what was filtered. Bot event records include route, device, location, ASN, and which detection layers matched.

## Detection Layers

Hygo runs all detection layers before making a final decision. A request can match multiple layers, and the bot event records every layer that matched.
...
Hygo combines these into a weighted score. Strong signals can identify bot traffic on their own, while weaker signals contribute supporting evidence.
...
Every detected bot carries the layers that caught it. Bots matched by one of Hygo's curated user-agent patterns carry three more fields, which is what the Bots dashboard groups by:
...
Hygo does not stop at the first matching layer. It runs every layer, records all matches, and then makes one final decision.
...
Hygo's tracker is JavaScript, so a crawler that fetches your HTML and executes nothing never reaches Hygo at all. Most AI training crawlers work exactly that way, which is why the Bots dashboard carries a note about its own coverage.
...
If you proxy Hygo through Cloudflare Workers, AWS CloudFront, Nginx, Caddy, or another reverse proxy, forward the original visitor IP:
