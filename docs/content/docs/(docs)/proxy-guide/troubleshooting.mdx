...
description: Common issues and solutions for Hygo proxy configurations
---

import { Callout } from 'fumadocs-ui/components/callout';
import { Steps } from 'fumadocs-ui/components/steps';

This guide covers common issues you might encounter when setting up a Hygo proxy and how to resolve them.
...
Verify Hygo backend is accessible:
...
- No tracking data appears in Hygo dashboard
...
Tracking endpoints need proper CORS headers. Hygo sets these automatically, but ensure your proxy preserves them:
...
// ✅ Correct - allow all origins (or use Hygo's headers)
...
**Caddy:**
```txt
reverse_proxy https://app.hygo.ai {
...
- Tracking works fine when the script talks to Hygo directly, without the proxy
...
#### Cause 2: Hygo sees your proxy's IP instead of the visitor's
...
1. Set `X-Forwarded-For` to the original visitor IP before forwarding to Hygo.
2. Enable the **First-Party Proxy** toggle in **Site Settings → Privacy & Security** so Hygo trusts the forwarded IP unconditionally.

<Callout type="warn">
Use `X-Forwarded-For`, not `X-Real-IP`. On Hygo Cloud, `X-Real-IP` never reaches the backend (our edge network consumes it), and AWS CloudFront cannot send it at all.
</Callout>
...
```javascript
{
  ...
  destination: `${process.env.NEXT_PUBLIC_HYGO_HOST}/api/session-replay/record/:siteId`,
}
```
...
**Symptoms:**
- Changed settings in Hygo dashboard
...
sudo rm -rf /var/cache/nginx/hygo/*
```

```javascript
// Node.js/Express with node-cache
cache.del(`hygo_config_${siteId}`);
...
```nginx
# Nginx with caching
proxy_cache_path /var/cache/nginx/hygo levels=1:2 keys_zone=hygo:10m;
...
```bash
# Test latency to Hygo
curl -w "@curl-format.txt" -o /dev/null -s https://app.hygo.ai/api/script.js
...
- Self-hosting Hygo closer to your users
- Using a CDN proxy
...
```javascript
async rewrites() {
    console.log('HYGO_HOST:', process.env.NEXT_PUBLIC_HYGO_HOST);
    // Should log the URL
    return [...];
}
```
...
```bash
curl -v https://app.hygo.ai/api/script.js
```
...
```bash
git add vercel.json
git commit -m "Add Hygo proxy config"
git push
```
...
- [ ] Client IP is forwarded in `X-Forwarded-For` (not just `X-Real-IP` — that header doesn't survive Hygo Cloud's edge)
...
5. **Verify directly** against `https://app.hygo.ai/api/script.js`
