...
description: Configure Laravel routes and middleware to proxy Hygo tracking
---
...
Laravel makes it straightforward to proxy Hygo tracking using routes and the HTTP client. This guide shows how to set up proxy endpoints in your Laravel application.
...
Laravel's HTTP client (built on Guzzle) provides an elegant way to proxy requests to Hygo servers while maintaining full control over headers, caching, and error handling.

**What you'll achieve:**
- Proxy all Hygo endpoints through your Laravel app
- Forward necessary headers for accurate tracking
- Optional caching with Laravel Cache
- Support all Hygo features
...
Add your Hygo host to `.env`:

```bash
# .env
HYGO_HOST=https://app.hygo.ai
# For self-hosted: HYGO_HOST=https://analytics.yourcompany.com
```
...
{
    private string $hygoHost;

    public function __construct()
    {
        $this->hygoHost = config('services.hygo.host', 'https://app.hygo.ai');
    }
...
Add Hygo configuration to `config/services.php`:
...
return [
    ...
    'hygo' => [
        'host' => env('HYGO_HOST', 'https://app.hygo.ai'),
    ],
];
```
...
4. **Verify in Hygo dashboard**: Data should appear

</Steps>

## How It Works

Laravel routes intercept requests to `/analytics/*` and forward them to Hygo:

1. Request to `/analytics/script.js` hits Laravel route
2. Controller forwards to `https://app.hygo.ai/api/script.js`
...
public function proxySessionReplay(Request $request, string $siteId)
{
    ...
    ForwardAnalyticsData::dispatch(
        "{$this->hygoHost}/api/session-replay/record/{$siteId}",
        $request->all(),
        [
...
**Problem**: Old script cached after Hygo update.

**Solution**:
Clear specific cache key:
```bash
php artisan cache:forget hygo_script_script.js
```
...
Cache::tags(['hygo', 'scripts'])->remember($cacheKey, 3600, function () {
    // ... fetch script
});

// Clear all Hygo cache
Cache::tags(['hygo'])->flush();
```
...
Http::pool(fn (Pool $pool) => [
    $pool->get("{$this->hygoHost}/api/script.js"),
    $pool->get("{$this->hygoHost}/api/replay.js"),
]);
```
