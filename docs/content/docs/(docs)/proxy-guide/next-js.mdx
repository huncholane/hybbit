...
description: Configure Next.js rewrites to proxy Hygo tracking and bypass ad blockers
---
...
Next.js provides a powerful [rewrites](https://nextjs.org/docs/app/api-reference/next-config-js/rewrites) feature that makes it easy to proxy Hygo tracking through your own domain. This guide works with both the App Router (Next.js 13+) and Pages Router.

<Callout type="info">
Enable **First-Party Proxy** in your Hygo **Site Settings → Privacy & Security**. Rewrites on this platform automatically forward the browser's headers and set `X-Forwarded-For` to the visitor IP — the toggle tells Hygo to trust that forwarded IP directly.
</Callout>

## Overview

Next.js rewrites allow you to map incoming request paths to different destination URLs transparently. This is perfect for proxying analytics since the browser sees requests to your domain while Next.js forwards them to Hygo's servers behind the scenes.

**What you'll achieve:**
- Serve Hygo scripts from your domain (e.g., `/analytics/script.js`)
...
Create or update your `.env` or `.env.local` file with your Hygo instance URL:
...
```bash
# .env.local
NEXT_PUBLIC_HYGO_HOST=https://app.hygo.ai
```
...
```bash
# .env.local
NEXT_PUBLIC_HYGO_HOST=https://analytics.yourcompany.com
```

Replace `analytics.yourcompany.com` with your actual Hygo instance domain.
...
This configuration proxies all Hygo endpoints including session replay and Web Vitals:
...
const nextConfig = {
  async rewrites() {
    const hygoHost = process.env.NEXT_PUBLIC_HYGO_HOST;

    if (!hygoHost) {
      console.warn('NEXT_PUBLIC_HYGO_HOST is not set. Hygo proxy will not work.');
      return [];
    }
    ...
    return [
      // Tracking scripts
      {
        ...
        destination: `${hygoHost}/api/script.js`,
      },
      {
        source: '/analytics/replay.js',
        destination: `${hygoHost}/api/replay.js`,
      },
      {
        source: '/analytics/metrics.js',
        destination: `${hygoHost}/api/metrics.js`,
      },
      // Tracking endpoints
      {
        source: '/analytics/track',
        destination: `${hygoHost}/api/track`,
      },
      {
        source: '/analytics/identify',
        destination: `${hygoHost}/api/identify`,
      },
      {
        source: '/analytics/session-replay/record/:siteId',
        destination: `${hygoHost}/api/session-replay/record/:siteId`,
      },
      {
        source: '/analytics/site/tracking-config/:siteId',
        destination: `${hygoHost}/api/site/tracking-config/:siteId`,
      },
    ];
  },
};
...
<Callout type="info">
**Custom Path**: You can change `/analytics` to any path you prefer (e.g., `/tracking`, `/stats`, `/hygo`). Just ensure you use the same path consistently in both your rewrites and script tag.
</Callout>

### Update Your Tracking Script

Update your script tag to load from the proxied path instead of directly from Hygo:
...
2. **Verify the script loads**:
   - You should see a request to `/analytics/script.js` (not `app.hygo.ai`)
...
3. **Verify tracking works**:
   ...
   - All requests should show your domain, not `app.hygo.ai`
4. **Check the Hygo dashboard**:
...
The Hygo tracking script has a built-in auto-discovery mechanism. When it loads from:
...
This means all tracking requests automatically go to `/analytics/track`, `/analytics/identify`, etc., which Next.js then rewrites to forward to Hygo's servers.
...
The Hygo server sets cache headers on its scripts, which Next.js respects:
...
4. Ensure your Hygo instance URL is correct in `.env.local`
...
**Problem**: Rewrites don't work, console shows warning about missing `NEXT_PUBLIC_HYGO_HOST`.
...
1. Add `NEXT_PUBLIC_HYGO_HOST` environment variable in your Vercel project settings
