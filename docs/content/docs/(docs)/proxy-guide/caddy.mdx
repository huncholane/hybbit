...
description: Configure Caddy reverse proxy to serve Hygo tracking with automatic HTTPS
---
...
Caddy is a powerful web server with automatic HTTPS that makes reverse proxying simple and secure. This guide shows how to configure Caddy to proxy Hygo tracking through your own domain.
...
This proxies all `/analytics/*` requests to `https://app.hygo.ai/api/*`.
...
3. **Open your website** in a browser with Developer Tools and verify:
   ...
   - Data appears in Hygo dashboard

</Steps>

## How It Works

Caddy's `reverse_proxy` directive forwards requests to Hygo's servers:
...
4. Forwards to `https://app.hygo.ai/api/script.js` with proper headers
5. Returns response to browser

The Hygo script auto-detects it's being served from your domain and sends all tracking to your domain's endpoints.
...
{$DOMAIN:yourdomain.com} {
    handle_path /analytics/* {
        reverse_proxy {$HYGO_HOST:https://app.hygo.ai} {
...
export HYGO_HOST=https://app.hygo.ai
caddy run
```
...
**Solution**:
1. Verify Hygo backend is accessible:
   ```bash
...
**Problem**: Requests go to wrong Hygo endpoints.
