...
description: Configure Nginx reverse proxy to serve Hygo tracking and bypass ad blockers
---
...
Nginx is a powerful web server and reverse proxy that makes it easy to proxy Hygo tracking through your own domain. This guide shows how to configure Nginx to forward tracking requests to Hygo servers while caching static scripts for better performance.

<Callout type="warn">
**Two settings make or break a first-party proxy:**

1. Enable **First-Party Proxy** in your Hygo **Site Settings → Privacy & Security**, so the visitor IP your proxy forwards in `X-Forwarded-For` is trusted directly.
2. Forward **all** of the browser's request headers, not a hand-picked few. Hygo's bot detection expects the headers every real browser sends (`Accept`, `Accept-Language`, `Accept-Encoding`, `Sec-Fetch-*`) — proxies that forward only `User-Agent` get their events accepted with `200 OK` but classified as bot traffic. Use `X-Forwarded-For` for the visitor IP; `X-Real-IP` does not survive Hygo Cloud's edge.
</Callout>

## Overview

Nginx's `proxy_pass` directive lets you forward requests from your domain to Hygo's servers. Combined with caching, this provides excellent performance and reliability for proxying analytics.

**What you'll achieve:**
- Proxy all Hygo endpoints through your domain
- Cache static scripts for better performance
- Forward necessary headers for accurate tracking
- Support all Hygo features (session replay, Web Vitals, custom events)
...
- Your Hygo instance URL:
  - **Cloud hosted**: `https://app.hygo.ai`
  - **Self-hosted**: Your instance URL
- Your Hygo site ID (found in your dashboard)
...
server {
    ...
    # Hygo Analytics Proxy
    # Main tracking script (cached)
    location = /analytics/script.js {
        proxy_pass https://app.hygo.ai/api/script.js;
        proxy_set_header Host app.hygo.ai;
    ...
    location = /analytics/replay.js {
        proxy_pass https://app.hygo.ai/api/replay.js;
        proxy_set_header Host app.hygo.ai;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location = /analytics/metrics.js {
        proxy_pass https://app.hygo.ai/api/metrics.js;
        proxy_set_header Host app.hygo.ai;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location = /analytics/track {
        proxy_pass https://app.hygo.ai/api/track;
        proxy_set_header Host app.hygo.ai;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location = /analytics/identify {
        proxy_pass https://app.hygo.ai/api/identify;
        proxy_set_header Host app.hygo.ai;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location ~ ^/analytics/session-replay/record/(.*)$ {
        proxy_pass https://app.hygo.ai/api/session-replay/record/$1;
        proxy_set_header Host app.hygo.ai;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location ~ ^/analytics/site/tracking-config/(.*)$ {
        proxy_pass https://app.hygo.ai/api/site/tracking-config/$1;
        proxy_set_header Host app.hygo.ai;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
...
<Callout type="info">
Replace `app.hygo.ai` with your self-hosted Hygo instance URL if you're not using the cloud version.
</Callout>
...
http {
    # Define cache zone for Hygo
    proxy_cache_path /var/cache/nginx/hygo
        levels=1:2
        keys_zone=hygo_cache:10m
        max_size=100m
        inactive=60m
        use_temp_path=off;
...
```bash
sudo mkdir -p /var/cache/nginx/hygo
sudo chown www-data:www-data /var/cache/nginx/hygo
```
...
Replace `YOUR_SITE_ID` with your actual site ID from the Hygo dashboard.
...
4. **Open your website** in a browser with Developer Tools and verify:
   ...
   - Data appears in your Hygo dashboard after 1-2 minutes

</Steps>

## How It Works

Nginx's `proxy_pass` directive transparently forwards requests to Hygo's servers:

1. Browser requests `https://yourdomain.com/analytics/script.js`
2. Nginx matches the location block and forwards to `https://app.hygo.ai/api/script.js`
...
The Hygo script auto-detects it's being served from your domain and sends all tracking data to your domain's `/analytics/*` endpoints.
...
location ~ ^/analytics/(script|replay|metrics)\.js$ {
    proxy_pass https://app.hygo.ai/api/$1.js;
    # ... proxy headers
    proxy_cache_valid 200 1h;
...
location ~ ^/analytics/(track|identify)$ {
    proxy_pass https://app.hygo.ai/api/$1;
    # ... proxy headers
...
Keep connections to Hygo backend alive for better performance:

```nginx
upstream hygo_backend {
    server app.hygo.ai:443;
    keepalive 32;
}

location /analytics/ {
    proxy_pass https://hygo_backend/api/;
...
```nginx
location ~ ^/analytics/session-replay/record/ {
    proxy_pass https://app.hygo.ai/api/session-replay/record/;
    # ... proxy headers
...
**Solution**:
1. Check Hygo backend is accessible:
   ```bash
   curl -I https://app.hygo.ai/api/script.js
   ```
...
**Problem**: Logs show SSL verification errors when connecting to Hygo.

**Solution**:
Add `proxy_ssl_server_name on` to enable SNI:
```nginx
location /analytics/ {
    proxy_pass https://app.hygo.ai/api/;
...
### Multiple Hygo Instances

If you need to proxy multiple Hygo instances:

```nginx
location /analytics-prod/ {
    proxy_pass https://prod.hygo.ai/api/;
    # ... proxy config
}

location /analytics-staging/ {
    proxy_pass https://staging.hygo.ai/api/;
    # ... proxy config
}
```
...
- [Nginx Custom Configuration Guide](/docs/self-hosting-guides/custom-nginx) - Self-hosting Hygo with Nginx
