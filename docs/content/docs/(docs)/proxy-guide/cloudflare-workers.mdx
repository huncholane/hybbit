...
description: Use Cloudflare Workers to proxy Hygo tracking at the edge
---
...
Cloudflare Workers provide a serverless way to proxy Hygo tracking at the edge, close to your users worldwide. This delivers excellent performance with minimal configuration.
...
2. Name it `hygo-proxy` (or your preferred name)
...
```javascript
// Cloudflare Worker for Hygo Analytics Proxy

const HYGO_HOST = 'https://app.hygo.ai';
...
async function handleRequest(request) {
  ...
  try {
    return await proxyToHygo(request, url);
  } catch (error) {
...
async function proxyToHygo(request, url) {
  // Map paths: /analytics/* → /api/*
  let hygoPath = url.pathname.replace('/analytics/', '/api/');

  // Build Hygo URL
  const hygoUrl = new URL(hygoPath, HYGO_HOST);
  hygoUrl.search = url.search; // Preserve query params
  ...
  // Build proxy request
  const proxyRequest = new Request(hygoUrl, {
...
```javascript
// Minimal Cloudflare Worker for Hygo
...
async function handleRequest(request) {
  ...
  if (url.pathname === '/analytics/script.js' || url.pathname === '/analytics/track') {
    const hygoPath = url.pathname.replace('/analytics/', '/api/');
    const hygoUrl = `${HYGO_HOST}${hygoPath}`;
...
async function handleRequest(request, env) {
  const HYGO_HOST = env.HYGO_HOST || 'https://app.hygo.ai';
  const url = new URL(request.url);
...
**Workers & Pages** → Your Worker → **Settings** → **Variables** → Add `HYGO_HOST`
...
3. **Verify in Hygo dashboard**: Data should appear
...
3. If not cached, proxies to `app.hygo.ai/api/script.js`
...
async function handleRequest(request) {
  ...
  try {
    const response = await proxyToHygo(request, url);
...
1. Verify cache key includes important parts:
   ```javascript
   const cacheKey = new Request(hygoUrl, request);
   ```
