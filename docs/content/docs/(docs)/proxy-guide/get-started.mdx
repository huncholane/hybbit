...
description: Framework-agnostic guide to proxy Hygo tracking through your own domain
---
...
This guide covers the core concepts for setting up a Hygo proxy with any reverse proxy or server framework. If your specific framework has a dedicated guide, we recommend following that instead for optimized instructions.
...
- **Your Hygo instance URL**:
  - Cloud: `https://app.hygo.ai`
  - Self-hosted: Your Hygo instance URL (e.g., `https://analytics.yourcompany.com`)
- **Your Hygo site ID** (found in your Hygo dashboard)

## Understanding the Endpoints

Hygo uses several endpoints for tracking. You can choose between a minimal setup or full proxy:
...
This setup supports all Hygo features including session replay, Web Vitals, and custom events.
...
Set up your reverse proxy to forward requests from your chosen path (e.g., `/analytics/*`) to the Hygo server.

**Generic proxy configuration pattern:**

```
Your domain path          →  Hygo backend
-----------------            ---------------
/analytics/script.js      →  https://app.hygo.ai/api/script.js
/analytics/track          →  https://app.hygo.ai/api/track
/analytics/replay.js      →  https://app.hygo.ai/api/replay.js
... and so on
```
...
In your Hygo dashboard, open **Site Settings → Privacy & Security** and enable **First-Party Proxy**. This tells Hygo that requests for this site arrive via your proxy, so the visitor IP in `X-Forwarded-For` is trusted directly instead of being inferred from network topology. Without it, Hygo may attribute traffic to your proxy server's IP — producing wrong geolocation and merging all visitors into a few users.

### Configure Caching (Optional but Recommended)

Implement caching to reduce load on Hygo servers and improve performance:
...
Change your script tag to load from your proxied domain instead of directly from Hygo:

**Before (direct loading):**
```html
<script src="https://app.hygo.ai/api/script.js" async data-site-id="YOUR_SITE_ID"></script>
```
...
<Callout type="info">
**How Auto-Discovery Works**: The Hygo script automatically extracts the analytics host from its own `src` attribute by splitting on `/script.js`. This means when it loads from `https://yourdomain.com/analytics/script.js`, it automatically sends all tracking data to `https://yourdomain.com/analytics/*` endpoints.
</Callout>
...
**CORS headers** (should be preserved from Hygo):
```
...
Forward the browser's original headers to the Hygo backend. The simplest and most robust approach is to pass through **every** incoming request header except `Host` and hop-by-hop headers (`Connection`, `Content-Length` — let your HTTP client recompute those).

Why this matters: Hygo's bot detection scores requests on headers that every real browser sends. A proxy that forwards only `User-Agent` and the visitor IP produces requests that look exactly like a script — the events return `200 OK` but are classified as bot traffic and never appear in your dashboard.
...
<Callout type="warn">
**IP Forwarding**: Hygo uses IP addresses for geolocation (country/city) and creating anonymous user identifiers (hashed IP + User Agent). If you don't forward the visitor IP in `X-Forwarded-For`, all traffic will appear to come from your proxy server's IP.
</Callout>

### Forwarded IP Headers

Set `X-Forwarded-For` to the original visitor IP before forwarding the request to Hygo, and enable the **First-Party Proxy** toggle in your site settings so Hygo trusts it unconditionally.

<Callout type="warn">
**Use `X-Forwarded-For`, not `X-Real-IP`.** On Hygo Cloud, `X-Real-IP` is consumed by our edge network (Cloudflare treats it as an internal header) and never reaches the backend — a proxy that relies on it will silently fall back to the wrong IP. `X-Forwarded-For` passes through intact. Coincidentally, AWS CloudFront also cannot send `X-Real-IP` (it's on the [origin custom-header denylist](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/add-origin-custom-headers.html#add-origin-custom-headers-denylist)). Setting `X-Real-IP` as well does no harm on self-hosted instances, but never make it the only IP header.
</Callout>
...
Configure your proxy to maintain persistent connections to Hygo's backend to reduce latency on repeated requests.
...
```
https://analytics.yourdomain.com/script.js  →  https://app.hygo.ai/api/script.js
https://analytics.yourdomain.com/track      →  https://app.hygo.ai/api/track
```
...
```
https://yourdomain.com/analytics/script.js  →  https://app.hygo.ai/api/script.js
https://yourdomain.com/analytics/track      →  https://app.hygo.ai/api/track
```
...
- Whitelist your Hygo backend IP if possible
...
2. **CORS errors**: Ensure CORS headers are preserved from Hygo backend
