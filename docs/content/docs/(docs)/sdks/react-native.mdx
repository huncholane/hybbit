...
The `@hygo/react-native` SDK lets you track screens, events, errors, and users in React Native apps. Mobile sites get an App Analytics dashboard mode with mobile-specific labels (e.g. screen views instead of pageviews).

## Creating a Mobile App Site

Before installing the SDK, create a site for your app in the Hygo dashboard:
...
`hygo.init()` **must** be called once before any other tracking methods. Calling other methods first throws an error.
</Callout>
...
| :-------------- | :----------------- | :--------------------------------------------------------------------------------- |
| `analyticsHost` | `string`           | URL of your Hygo instance's API (e.g., `https://hygo.yourdomain.com/api`).     |
| `siteId`        | `string \| number` | The Site ID for your app obtained from your Hygo instance.                       |
...
| `storage`               | `HygoStorage` | in-memory   | Storage adapter used to persist the anonymous visitor ID and identified user ID. Pass `AsyncStorage` or any compatible object.              |
| `storageKeyPrefix`      | `string`        | `"@hygo"` | Prefix for storage keys. Keys are namespaced per site: `{prefix}:{siteId}:anonymous-id`.                                                    |
...
| Capture Errors         | When disabled, calls to `hygo.error()` are silently ignored.                |
...
```typescript
hygo.screen(name: string, properties?: TrackProperties, context?: TrackContext): Promise<void>;
```
...
```tsx
await hygo.screen("Checkout");

// With properties and an explicit path
await hygo.screen("Product", { productId: "sku-123" }, { pathname: "/products/sku-123" });
```

You can also track a raw path directly, mirroring the web SDK:

```typescript
hygo.pageview(path?: string, context?: TrackContext): Promise<void>;
```
...
```typescript
hygo.createNavigationTracker(options?: NavigationTrackerOptions): NavigationTracker;
```
...
```typescript
hygo.event(name: string, properties?: TrackProperties, context?: TrackContext): Promise<void>;
```
...
await hygo.event("purchase_completed", {
  orderId: "order-123",
  total: 49.99,
  currency: "USD",
});
...
Capture handled errors with additional context. Error tracking must be enabled in your Hygo dashboard (Capture Errors) — when disabled, `error()` calls are ignored.

### Syntax

```typescript
hygo.error(error: Error | unknown, properties?: TrackProperties, context?: TrackContext): Promise<void>;
```
...
try {
  ...
  await hygo.error(error, { component: "checkout", orderId: order.id });
  showErrorMessage();
}
```

<Callout type="info">
Unlike the web SDK, the React Native SDK does not automatically capture uncaught exceptions — wrap the code you care about and call `hygo.error()` yourself.
</Callout>
...
```typescript
hygo.identify(userId: string, traits?: Record<string, unknown>): Promise<void>;
hygo.setTraits(traits: Record<string, unknown>): Promise<void>;
hygo.clearUserId(): Promise<void>;
hygo.getUserId(): string | null;
```

```tsx
// After login
await hygo.identify("user_123", { plan: "pro", signupDate: "2026-01-15" });

// Update traits later without changing the ID
await hygo.setTraits({ plan: "enterprise" });

// After logout
await hygo.clearUserId();
```
...
- Uncaught errors are not captured automatically — use `hygo.error()` in your own error boundaries and catch blocks.
