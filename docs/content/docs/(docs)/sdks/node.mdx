...
```typescript
import { Hygo } from "@hygo/node";

const hygo = new Hygo(config: HygoConfig);
```
...
The `Hygo` constructor will throw an error if required configuration options (`analyticsHost`, `siteId`, `apiKey`) are missing or invalid.
</Callout>
...
| :-------------- | :----------------- | :------ | :----------------------------------------------------------------------------------------------- |
| `analyticsHost` | `string`           | -       | **Required.** URL of your Hygo analytics instance (e.g., `https://hygo.yourdomain.com/api`). |
| `siteId`        | `string \| number` | -       | **Required.** The Site ID for your website obtained from your Hygo instance.                   |
...
```typescript
await hygo.pageview(payload?: Payload);
```
...
```typescript
await hygo.event(eventName: string, payload?: Payload, properties?: TrackProperties);
```
...
async function registerUser(userData, req) {
  ...
  await hygo.event("user_registered",
    {
...
async function processDailyReport(reportId) {
  ...
  await hygo.event("report_generated",
    {},
    {
      reportId: reportId,
      status: "success",
