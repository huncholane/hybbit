...
Hygo can automatically capture common user interactions without requiring any code changes. These features can be enabled in your [Site Settings](/docs/site-settings) under the "Auto Capture" section.
...
When enabled, Hygo automatically tracks clicks on links that navigate to external domains.
...
You can add custom properties to auto-tracked button clicks using `data-hygo-prop-*` attributes:

```html
<button
  data-hygo-prop-category="pricing"
  data-hygo-prop-variant="blue"
>
  Buy Now
</button>
```
...
If you want to track a button with a custom event instead of the automatic `button_click`, add the `data-hygo-event` attribute:

```html
<!-- This button will fire a custom "signup" event, NOT a button_click -->
<button data-hygo-event="signup" data-hygo-prop-plan="pro">
  Sign Up
</button>
```
...
You can add custom properties using `data-hygo-prop-*` on the form element:

```html
<form
  id="signup"
  data-hygo-prop-form-type="registration"
  data-hygo-prop-variant="a"
>
  ...
</form>
```
...
You can add custom properties using `data-hygo-prop-*` on the input element:

```html
<input
  name="email"
  data-hygo-prop-field-category="contact"
/>

<select
  name="country"
  data-hygo-prop-section="demographics"
>
  ...
</select>
```
