...
description: How to identify and track logged-in users with Hygo
---
...
Hygo allows you to identify logged-in users and associate their activity with a persistent identity across devices and sessions. This enables powerful user-level analytics, including tracking user journeys, understanding retention, and personalizing experiences.

![identify users](/docs/users.png)

## How It Works

By default, Hygo assigns an anonymous ID to each visitor based on their device fingerprint. When you call `identify()`, you link this anonymous visitor to a known user ID, allowing you to:
...
When a new alias is created (i.e., the first time `identify()` is called for a given device), Hygo retroactively updates all past anonymous events from that device to include the identified user ID. This means pre-login pageviews and events are attributed to the user once they identify themselves.

## Special Trait Fields

When identifying users, certain trait fields are treated specially by Hygo:
...
### `window.hygo.identify(userId, traits?)`
...
- `traits` (Object, Optional): An object containing user metadata like email, name, username, or any custom fields. This data is stored in Hygo and can be used to identify users in the dashboard. Setting a trait to `null` removes it from the user's profile.

```javascript
// Identify a user when they log in (without traits)
window.hygo.identify("user_12345");
...
function handleLogin(userData) {
  ...
  window.hygo.identify(userData.id, {
...
function handlePlanUpgrade(newPlan) {
  ...
  // Track upgrade event
  window.hygo.event("Plan Upgraded", {
    new_plan: newPlan
  });
}
...
function checkUserStatus() {
  const userId = window.hygo.getUserId();
  if (userId) {
