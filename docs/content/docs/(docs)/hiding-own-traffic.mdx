...
There are several ways to disable Hygo tracking to hide your own visits from analytics.
...
<Tabs items={['IP Exclusion', 'Browser Extensions', 'localStorage Method']}>
  <Tab>
    **IP Exclusion (Recommended)**: Block traffic from specific IP addresses or ranges at the server level for complete accuracy.
    ...
    1. **Navigate to Site Settings**: Go to your site's settings page in the Hygo dashboard
    ...
    Hygo supports multiple IP exclusion formats:
  ...
  <Tab>
    **Browser Extensions**: Easy client-side solution using community-maintained browser extensions.
    ...
    - **[Firefox Extension](https://addons.mozilla.org/en-US/firefox/addon/hygo-opt-out/)**: Install from Firefox Add-ons

    These extensions automatically disable Hygo tracking on all websites where you have Hygo installed, without requiring any manual configuration.

    <Callout type="info">
    **Self-hosted users**: These extensions require Hygo version 1.1.0 or higher. If you're running an older version, please update your installation or use the localStorage method.
    </Callout>
...
- **Browser extensions** offer convenience by automatically working across all your Hygo-enabled websites  
