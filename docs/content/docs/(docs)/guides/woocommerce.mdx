---
title: WooCommerce
description: Integrate Hygo Analytics with your WooCommerce store
---
...
This guide explains how to integrate Hygo with your WooCommerce store (running on WordPress) to track e-commerce events like product views, add to carts, and purchases, in addition to standard page view analytics.

## Prerequisites

1.  **Hygo Installed on WordPress**: Ensure you have already installed the Hygo tracking script on your WordPress site using one of the methods described in our [WordPress integration guide](/docs/guides/wordpress). This guide focuses on adding e-commerce event tracking.
...
WooCommerce provides various action hooks that allow you to trigger JavaScript when specific e-commerce events occur. We'll use these to send data to Hygo.
...
add_action( 'woocommerce_after_single_product_summary', 'hygo_track_view_product', 10 );
function hygo_track_view_product() {
    ...
    <script type="text/javascript">
        if (typeof window.hygo !== 'undefined') {
            window.hygo.event('view_item', <?php echo json_encode($product_data); ?>);
        }
    </script>
    <?php
}
...
```php
// Add to your child theme's functions.php or a custom plugin
add_action( 'woocommerce_after_add_to_cart_button', 'hygo_track_add_to_cart_script' );
function hygo_track_add_to_cart_script() {
    global $product;
    if ( ! $product instanceof WC_Product ) {
        return;
    }
        ...
        if (form) {
            form.addEventListener('submit', function() {
                ...
                if (typeof window.hygo !== 'undefined') {
                    ...
                    window.hygo.event('add_to_cart', eventData);
                }
            });
        }
    });
...
add_action( 'wp_footer', 'hygo_ajax_add_to_cart_tracker' );
function hygo_ajax_add_to_cart_tracker() {
    if ( is_admin() ) return;
    ?>
    <script type="text/javascript">
        jQuery(document.body).on('added_to_cart', function(event, fragments, cart_hash, $button) {
            if (typeof window.hygo !== 'undefined' && $button && $button.length) {
...
```php
// Add to your child theme's functions.php or a custom plugin
add_action( 'woocommerce_before_checkout_form', 'hygo_track_begin_checkout', 5 );
function hygo_track_begin_checkout() {
    ...
    <script type="text/javascript">
        if (typeof window.hygo !== 'undefined') {
            window.hygo.event('begin_checkout', <?php echo json_encode($checkout_data); ?>);
        }
    </script>
    <?php
}
...
```php
// Add to your child theme's functions.php or a custom plugin
add_action( 'woocommerce_thankyou', 'hygo_track_purchase', 10, 1 );
function hygo_track_purchase( $order_id ) {
    if ( ! $order_id ) {
        return;
    }
    ...
    <script type="text/javascript">
        if (typeof window.hygo !== 'undefined') {
            window.hygo.event('purchase', <?php echo json_encode($purchase_data); ?>);
        }
    </script>
    <?php
}
...
*   **Script Loading Order**: Ensure the main Hygo tracking script (added via the WordPress integration guide) is loaded before these event-specific scripts attempt to call `window.hygo`. Placing the main script in the `<head>` usually ensures this.
...
By implementing these tracking snippets, you can gain valuable insights into your WooCommerce store's performance using Hygo.