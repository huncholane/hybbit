---
title: Astro
description: Integrate Hygo Analytics with your Astro application for content-rich websites
---
...
This guide explains how to integrate Hygo with your Astro application for analytics tracking. Astro is a web framework for building content-rich websites, delivering minimal JavaScript by default.

## How to Add Hygo to Your Astro Site

The most common way to add a global script like Hygo's to an Astro site is by including it in a common layout component that is used across your pages, or directly within the `<head>` of your pages if you don't use a shared layout.
...
Open your layout file and add the Hygo script tag within the `<head>` section.
...
const hygoSiteId = "YOUR_SITE_ID"; // Replace with your actual Site ID
---
<!doctype html>
<html lang="en">
	<head>
    ...
    <!-- Hygo Tracking Snippet -->
    <script
      src="https://app.hygo.ai/api/script.js"
      data-site-id={hygoSiteId}
      defer
    ></script>
	</head>
	<body>
...
After building and deploying your Astro site, open your Hygo dashboard to check if data is being received. You can also inspect your browser's network tab to confirm that `script.js` is loaded on your pages.
</Step>
</Steps>

<Callout type="info">
  **Page View Tracking in Astro**:
  - **MPA (Multi-Page Application) Mode**: By default, Astro builds multi-page applications. When a user navigates between different pages, a full page load occurs, and Hygo's script will naturally track this as a new page view.
  - **View Transitions**: If you are using Astro's [View Transitions API](https://docs.astro.build/en/guides/view-transitions/) for client-side navigation effects, Hygo's script should generally still detect these as page views if the URL changes. However, if you notice issues, you might need to manually trigger page views. You can listen to Astro's `astro:after-swap` event:
    ```javascript
...
<script>
  document.getElementById('my-astro-button')?.addEventListener('click', () => {
    if (window.hygo) {
      window.hygo.event('astro_button_click', { source: 'MyButton.astro' });
    }
  });
</script>
```
...
export default function MyReactIsland() {
  const handleClick = () => {
    if (window.hygo) {
      window.hygo.event('react_island_click', { island: 'MyReactIsland' });
    }
  };
