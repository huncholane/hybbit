---
title: WordPress
description: Integrate Hygo Analytics with your WordPress site
---
...
You can integrate Hygo into your WordPress site to track user behavior and gather analytics. There are several ways to do this depending on whether you're using a hosted WordPress.com site or a self-hosted WordPress.org installation.
...
(function() {
  ...
  s.src = 'https://app.hygo.ai/api/script.js?siteId=YOUR_SITE_ID';
  s.defer = true;
...
3. Replace `YOUR_SITE_ID` with your actual Site ID from your Hygo dashboard
...
Visit your site in a new browser tab, then check your Hygo dashboard to confirm data is being received.
</Step>
</Steps>
...
add_action('wp_head', 'add_hygo_analytics');
```
...
3. Create a new folder called `hygo-analytics`
4. Inside that folder, create a file called `hygo-analytics.php` with this content:

```php
<?php
/**
 * Plugin Name: Hygo Analytics
 * Description: Adds Hygo Analytics tracking to your WordPress site
 * Version: 1.0
 */

function hygo_add_tracking_script() {
    ?>
    <script>
    (function() {
      var s = document.createElement('script');
...
add_action('wp_head', 'hygo_add_tracking_script');
```

5. Replace `YOUR_SITE_ID` with your actual Site ID
6. Go to **Plugins** in WordPress and activate "Hygo Analytics"
</Tab>
</Tabs>
...
3. Search for `hygo` - you should see the script tag
...
Many WordPress caching plugins aggressively cache pages. After adding Hygo:
...
2. **Exclude the script from optimization:**
   Some optimization plugins try to defer, delay, combine, or minify scripts. "Delay JavaScript execution" features in particular rebuild the script tag and drop its `data-*` attributes, which is why the snippets on this page pass the site ID in the URL (`?siteId=`) instead of a `data-site-id` attribute. If Hygo still isn't working, exclude `app.hygo.ai` in these plugins:
   - **Autoptimize**: Add `app.hygo.ai` to the exclusion list
   - **WP Rocket**: Exclude from JavaScript optimization and from "Delay JavaScript execution"
   - **Perfmatters / FlyingPress**: Add `app.hygo.ai` to the delay-JS exclusions
...
Security plugins like Wordfence or Sucuri may block external scripts. Add `app.hygo.ai` to your allowlist if tracking isn't working.
