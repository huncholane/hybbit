---
title: Docusaurus
description: Integrate Hygo Analytics with your Docusaurus v2+ website
---
...
This guide explains how to integrate Hygo with your Docusaurus v2+ website for analytics tracking. Docusaurus is a static site generator that builds a single-page application with React.

## How to Add Hygo to Your Docusaurus Site

Docusaurus provides a configuration option to add scripts or tags to the `<head>` of every page, which is the recommended way to include Hygo.

<Steps>
<Step>
### Retrieve Your Tracking Script Details

Navigate to your Hygo dashboard to obtain your **Site ID**. The script source will be `https://app.hygo.ai/api/script.js`.
</Step>
...
You can add the Hygo script using the `scripts` array or the `headTags` array within your site configuration. Using `scripts` is often cleaner for simple script additions.
...
export default function MyCustomButton({ children, eventName, eventData }) {
  const handleClick = () => {
    if (typeof window !== 'undefined' && window.hygo) {
      window.hygo.event(eventName || 'docusaurus_custom_click', eventData || {});
    }
  };
...
```javascript filename="src/client-modules/hygo-events.js"
...
export function onRouteUpdate({location, previousLocation}) {
  ...
  if (typeof window !== 'undefined' && window.hygo && location.pathname !== previousLocation?.pathname) {
    // Hygo should auto-track pageviews, but if needed:
    // window.hygo.pageview();
  }
  ...
  document.querySelectorAll('a[target="_blank"]').forEach(link => {
    link.addEventListener('click', () => {
      if (window.hygo) {
        window.hygo.event('external_link_click', { href: link.href });
      }
    });
  });
}
...
Always ensure `window.hygo` is available before calling its methods in client-side scripts.