---
title: "Shopify"
description: "Integrate Hygo with your Shopify store to capture events, track conversions, and analyze user behavior."
---
...
Hygo can be integrated with your Shopify store to capture events, track conversions, and analyze user behavior.

## How to Add Hygo to Shopify

<Steps>
<Step>
### Retrieve Your Tracking Script

Navigate to your Hygo dashboard to obtain your code snippet.
...
To get the most out of Hygo with Shopify, you should track key e-commerce events. This typically involves adding small Liquid and JavaScript snippets to your theme files.
...
<script type="text/javascript">
  if (typeof window.hygo !== 'undefined' && {{ product | json }}) {
    const hygoProduct = {{ product | json }};
    const hygoCurrentVariant = {{ product.selected_or_first_available_variant | json }};

    window.hygo.event('view_item', {
      item_id: hygoCurrentVariant.sku || hygoCurrentVariant.id,
      item_name: hygoProduct.title,
      item_variant: hygoCurrentVariant.title === 'Default Title' ? null : hygoCurrentVariant.title,
      price: parseFloat(hygoCurrentVariant.price) / 100, // Price is in cents
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const trackAddToCart = function() {
    if (typeof window.hygo !== 'undefined' && {{ product | json }}) {
      ...
      window.hygo.event('add_to_cart', {
        item_id: hygoCurrentVariant.sku || hygoCurrentVariant.id,
        item_name: hygoProduct.title,
        item_variant: hygoCurrentVariant.title === 'Default Title' ? null : hygoCurrentVariant.title,
        price: parseFloat(hygoCurrentVariant.price) / 100,
        currency: {{ cart.currency.iso_code | json }},
...
Shopify's checkout process is on a separate domain (`checkout.shopify.com`), and direct script injection into the checkout pages (beyond the "Order status" page) is limited for most plans. However, Hygo's script, if loaded on your main store, should persist session information.
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (checkoutButton && typeof window.hygo !== 'undefined' && {{ cart | json }}) {
    checkoutButton.addEventListener('click', function() {
      const hygoCart = {{ cart | json }};
      let items = [];
      hygoCart.items.forEach(function(item) {
        items.push({
      ...
      window.hygo.event('begin_checkout', {
        currency: hygoCart.currency,
        value: parseFloat(hygoCart.total_price) / 100,
        items: items
      });
    });
  }
...
<Callout type="warn">
  Hygo limits an event's `properties` to **2048 characters** once serialized to
...
`window.hygo`** on the storefront. Instead, subscribe to the standard
`checkout_completed` event and send the purchase directly to Hygo's tracking
endpoint with `fetch`.

**1. Create a custom pixel:**
- In your Shopify Admin, go to **Settings > Customer events**.
- Click **Add custom pixel**, give it a name (e.g. `Hygo`), and click **Add pixel**.
...
analytics.subscribe("checkout_completed", (event) => {
  ...
  // Keep the payload small — Hygo rejects event properties over 2048 chars
  ...
  fetch("https://app.hygo.ai/api/track", {
    method: "POST",
...
- The pixel POSTs a `custom_event` straight to Hygo's `/api/track` endpoint
  because the sandbox has no access to the storefront's `window.hygo`.
...
*   **Currency Conversion**: Prices in Shopify Liquid objects are often in cents. Ensure you divide by 100 to get the decimal value for Hygo.
...
*   **Shopify Customer Events**: For more robust server-side or client-side event tracking, especially for `add_to_cart` and `begin_checkout` without relying on DOM manipulation, explore [Shopify Customer Events](https://shopify.dev/docs/api/customer-events). This allows you to subscribe to events like "product added to cart" or "checkout started" and send data to Hygo via a custom pixel or by triggering client-side JavaScript. This is a more advanced but often more reliable method.
*   **Testing**: Thoroughly test each event in your Hygo dashboard after implementation. Use your browser's developer console to check for JavaScript errors.