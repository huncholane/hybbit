...
Experiments are A/B tests built on top of Hygo. Each experiment splits your visitors across the variants of a [multivariate feature flag](/docs/feature-flags), then measures how often each variant completes a [conversion goal](/docs/goals). Hygo calculates the conversion rate for every variant, the **lift** relative to your control, and how confident you can be that the difference is real.
...
| **Exposure** | When your code reads the flag with `hygo.flag()`, Hygo records an exposure for that variant. The number of distinct sessions exposed to a variant is the denominator for its conversion rate. |
...
Open your site in the Hygo dashboard, go to **Experiments**, and click **New experiment**. A guided wizard walks you through each step.
...
Review the assignment flag and goal, then create the experiment. Hygo creates any new flag or goal you configured, connects them, and shows you the **implementation code** for the flag. The experiment stays in **draft** until you start it from the experiment list.
</Step>
</Steps>
...
window.hygo.onReady((hygo) => {
  const variant = hygo.flag("checkout_cta", "control");
...
window.hygo.onReady((hygo) => {
  const variant = hygo.flag("checkout_cta", "control") as ExperimentVariant;
...
```javascript
window.hygo.onReady((hygo) => {
  hygo.event("signup_completed");
});
```

**Page goals** need no extra code — Hygo counts sessions that reach the goal's path automatically.

<Callout type="info">
Use a stable variant for the `hygo.flag()` fallback (usually `"control"`) so visitors still get a sensible experience before the flag resolves or if the experiment is paused.
</Callout>
...
| **Confidence** | How confident Hygo is that the variant's difference from the control is real, based on a two-proportion comparison. |

Hygo summarizes the state of each experiment at a glance:
