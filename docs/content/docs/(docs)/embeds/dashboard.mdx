...
The dashboard embed lets you place the main Hygo analytics dashboard on another site with a self-contained iframe. Dashboard embeds use private-link routes with `embed=true`.

## Try It Out

Configure the private-link dashboard embed URL below. The generated snippet uses the same private-link route created in the Hygo dashboard.
...
- **Footer**: the Hygo footer is hidden automatically in embed mode.

## Example

```html
<iframe
  src="https://app.hygo.ai/YOUR_SITE_ID/YOUR_PRIVATE_LINK_KEY/main?embed=true&theme=system&hideSidebar=true"
