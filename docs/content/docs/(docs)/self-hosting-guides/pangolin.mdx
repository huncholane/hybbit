...
description: Set up Hygo with Pangolin as a self-hosted reverse proxy using Newt
---

import { Steps, Step } from 'fumadocs-ui/components/steps';
import { Callout } from 'fumadocs-ui/components/callout';

This guide shows you how to set up Hygo with [Pangolin](https://github.com/fosrl/pangolin), a self-hosted reverse proxy, using a Newt container inside the Docker Compose stack. This approach avoids exposing ports directly on your host network.
...
Create a `.env` file in your Hygo directory:
...
Create a `docker-compose.yml` file in the same directory. This includes a Newt container that connects Pangolin to your Hygo services:

```yaml
services:
  newt:
    image: fosrl/newt
    container_name: newt-hygo
...
In your Pangolin dashboard, create a new site for your Hygo domain and configure two resources:

1. **Client resource** — routes the root domain to the Hygo frontend:
   - Target: `client` (the Docker container hostname)
   - Port: `3002`

2. **Backend resource** — routes `/api` traffic to the Hygo backend:
...
```bash
docker compose logs -f newt-hygo
```
...
For a visual walkthrough of setting up Hygo with Pangolin, see the official Pangolin YouTube guide:
