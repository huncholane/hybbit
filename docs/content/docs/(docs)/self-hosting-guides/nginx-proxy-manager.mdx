...
description: Set up Hygo with Nginx Proxy Manager without exposing ports directly to the host
---
...
This guide shows you how to set up Hygo with Nginx Proxy Manager without exposing ports directly to the host.
...
<Callout type="info">
This setup assumes you have NPM running and are capable of creating SSL certificates. You can also use any other reverse proxy as long as you can put it on the same Docker network as Hygo.
</Callout>
...
Create a new directory for your Hygo installation and create a `docker-compose.yml` file:

```yaml
services:
  hygo_clickhouse:
    image: clickhouse/clickhouse-server:26.3.17.4
    container_name: hygo_clickhouse
    volumes:
  ...
  hygo_backend:
    ...
    container_name: hygo_backend
    environment:
      - NODE_ENV=production
      - CLICKHOUSE_HOST=http://hygo_clickhouse:8123
      - CLICKHOUSE_DB=${CLICKHOUSE_DB:-analytics}
      - CLICKHOUSE_PASSWORD=${CLICKHOUSE_PASSWORD:-frog}
      - POSTGRES_HOST=hygo_postgres
      - POSTGRES_PORT=5432
      - POSTGRES_DB=${POSTGRES_DB:-analytics}
      - POSTGRES_USER=${POSTGRES_USER:-frog}
      - POSTGRES_PASSWORD=${POSTGRES_PASSWORD:-frog}
      - REDIS_HOST=hygo_redis
      - REDIS_PORT=6379
...
1. **Create Proxy Host**:
   ...
   - Scheme: `http`
   - Forward Hostname/IP: `hygo_client`
   - Forward Port: `3002`
...
2. **Add Custom Location**:
   - Location: `/api`
   - Scheme: `http`
   - Forward Hostname/IP: `hygo_backend`
   - Forward Port: `3001`

3. **Advanced Configuration** (in the custom location):
   ```nginx
   location /api/ {
       proxy_pass http://hygo_backend:3001;
...
ingress:
  - hostname: your.domain.com
    path: /api
    service: http://hygo_backend:3001
  - hostname: your.domain.com
    service: http://hygo_client:3002
...
- Verify both NPM and Hygo are on the same network
...
**502 Bad Gateway**:
- Verify Hygo services are healthy: `docker compose ps`
...
# Specific service
docker compose logs -f hygo_backend
