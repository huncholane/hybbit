...
**Cause:** Hygo is seeing the proxy IP instead of the original visitor IP. This is common when a first-party proxy is hosted on CDN or cloud infrastructure. Those networks use hosting/datacenter ASNs, so traffic can look like bot infrastructure if the visitor IP is not passed in a trusted way.

Set `X-Forwarded-For` or `X-Real-IP` to the original visitor IP before forwarding the request to Hygo. Hygo reads those headers before the connecting edge IP.
