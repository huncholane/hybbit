...
For accurate tracking, ensure these headers are forwarded to the Hygo backend:
...
<Callout type="warn">
**IP Forwarding**: Hygo uses IP addresses for geolocation (country/city) and creating anonymous user identifiers (hashed IP + User Agent). If you don't forward the `X-Forwarded-For` or `X-Real-IP` header, all traffic will appear to come from your proxy server's IP.
</Callout>

### Forwarded IP Headers

If your proxy runs on infrastructure such as Cloudflare Workers, AWS CloudFront, Fastly, or another edge/CDN provider, make sure the proxy sets `X-Forwarded-For` or `X-Real-IP` to the original visitor IP. Hygo reads these headers before the connecting edge IP, so this prevents traffic from being geolocated to the proxy or classified by the proxy's ASN.
