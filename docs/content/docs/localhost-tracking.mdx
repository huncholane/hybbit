...
By default, Hygo validates tracking events come from your registered domain. During development, you can use API keys to track from localhost.
...
export function Analytics() {
  ...
  return (
    <script
      ...
      {...(isDev && { 'data-api-key': process.env.NEXT_PUBLIC_HYGO_API_KEY })}
    />
  );
}
```
...
if (process.env.NODE_ENV === 'development') {
  script.setAttribute('data-api-key', process.env.HYGO_API_KEY);
}
