...
Access your site settings by clicking the settings icon in your site's Hygo dashboard.
...
Hygo stores user IDs as a hashed combination of IP + user agent. This allows identifying users across sessions, but may be considered storing personally identifiable data under GDPR.

Enable "User ID Salting" to have Hygo forget about unique users every day. If a user visits 10 times in one day, they count as one user with 10 sessions. But if they visit once a day for 10 days, they appear as a new user each day.
...
Enable "Block Bot Traffic" to prevent detected bot traffic from being tracked in your normal analytics. See [Bot detection](/docs/bot-detection) for how Hygo identifies and stores bot traffic.
...
Show Hygo data on another site with an embeddable iframe.
