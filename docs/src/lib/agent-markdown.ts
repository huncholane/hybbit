...
const SITE_ORIGIN = "https://hygo.ai";
const MAX_MARKDOWN_SOURCE_BYTES = 2_000_000;
const REDIRECT_STATUSES = new Set([301, 302, 303, 307, 308]);

export const homepageMarkdown = `# Hygo

Hygo is an open-source, cookieless web and product analytics platform. It replaces Google Analytics with one readable dashboard and a lightweight 18 KB tracking script. Hygo is designed for privacy-conscious teams: no cookies, no consent banner, GDPR and CCPA compliant, and available as managed EU-hosted cloud or a self-hosted deployment.

## What Hygo helps you understand
...
Hygo records Core Web Vitals from real visits and breaks performance down by route, country, browser, operating system, and device. Error tracking connects JavaScript failures to affected sessions so teams can move from a trend to the visits that explain it.

## Built for people and agents

Hygo exposes a REST API and a hosted MCP server. Agents can read analytics, investigate errors and funnels, and manage goals with the same permissions as a teammate. API requests use a Bearer token, and the published OpenAPI document describes the available operations.
...
- [Live demo](https://demo.hygo.ai)
- [Source code](https://github.com/hygo-ai/hygo)
...
The requested path does not exist on Hygo.

## Where to look next

- [Hygo home](${SITE_ORIGIN}/)
- [Documentation](${SITE_ORIGIN}/docs)
- [Sitemap](${SITE_ORIGIN}/sitemap.xml)
...
export async function getMarkdownRepresentation(
  ...
  try {
    ...
    return markdownResponse(
      "# Upstream page unavailable\n\nThe HTML representation could not be rendered. Try the [documentation](https://hygo.ai/docs) or retry this request.",
      502,
  ...
  try {
    ...
    return markdownResponse(
      "# Representation unavailable\n\nThe rendered page exceeded the safe conversion limit. See the [agent index](https://hygo.ai/llms.txt).",
      502,
      { "Cache-Control": "no-store" }
    );
  }
  if (!markdown) {
    return markdownResponse(
      "# Representation unavailable\n\nThe page did not contain readable main content. See the [agent index](https://hygo.ai/llms.txt).",
      502,
      { "Cache-Control": "no-store" }
    );
  }
