...
describe("agent-readable HTTP behavior", () => {
  ...
  it("forwards the original path and query to the Markdown renderer", () => {
    const { rewriteUrl, requestHeaders } = markdownProxyTarget(
      "https://hygo.ai/docs/api/getting-started?source=audit",
      { Accept: "text/markdown" }
    );

    assert.equal(
      rewriteUrl.toString(),
      "https://hygo.ai/api/agent/markdown?path=%2Fdocs%2Fapi%2Fgetting-started&query=%3Fsource%3Daudit"
    );
    assert.equal(requestHeaders.get("X-Hygo-Markdown-Path"), "/docs/api/getting-started");
    assert.equal(requestHeaders.get("X-Hygo-Markdown-Query"), "?source=audit");
  });

  it("serves substantial homepage Markdown with a real heading hierarchy", () => {
    assert.ok(homepageMarkdown.length >= 500);
    assert.match(homepageMarkdown, /^# Hygo/m);
    assert.match(homepageMarkdown, /^## /m);
    assert.match(homepageMarkdown, /^### /m);
    assert.match(homepageMarkdown, /https:\/\/hygo\.com\/docs/);
  });
  ...
  it("returns a recoverable Markdown 404 for an unknown page", async () => {
    ...
    assert.match(body, /\[Sitemap\]\(https:\/\/hygo\.com\/sitemap\.xml\)/);
    assert.match(body, /\[Agent index\]\(https:\/\/hygo\.com\/llms\.txt\)/);
    assert.match(body, /\[API specification\]\(https:\/\/hygo\.com\/openapi\.json\)/);
  });
  ...
  it("keeps Markdown source fetches on the configured origin", () => {
    ...
    const source = markdownSourceUrl("https://hygo.ai", "/docs/api/getting-started", "?source=agent");
    assert.equal(source.origin, "https://hygo.ai");
  ...
  it("generates OpenAPI 3.1 for the documented API and structured errors", () => {
    ...
    assert.equal(document.servers[0].url, "https://app.hygo.ai");
