...
function docsUrl(root: string, file: string): string {
  const slug = relative(root, file)
    ...
    .join("/")
    .replace(/\.mdx$/, "");
  return `https://hygo.ai/docs/api/${slug}`;
}
...
function pathParameters(path: string): Array<Record<string, unknown>> {
  return [...path.matchAll(/\{([A-Za-z][A-Za-z0-9_]*)\}/g)].map(match => ({
    description: match[1] === "site" ? "Numeric Hygo Site ID or string identifier." : `${match[1]} identifier.`,
    in: "path",
    name: match[1],
    required: true,
    schema:
...
export function buildOpenApiDocument(): OpenApiDocument {
  ...
  return {
    ...
    info: {
      ...
      version: "2.8.0-beta",
      description:
        "REST API for Hygo web and product analytics. The API is in beta and may introduce breaking changes.",
      termsOfService: "https://hygo.ai/terms-and-conditions",
      contact: { name: "Hygo", url: "https://hygo.ai/contact" },
      license: {
        name: "GNU Affero General Public License v3.0",
        identifier: "AGPL-3.0-only",
      },
    },
    servers: [{ url: "https://app.hygo.ai", description: "Hygo Cloud API" }],
    ...
    externalDocs: {
      description: "Hygo API documentation",
      url: "https://hygo.ai/docs/api/getting-started",
    },
  };
}
