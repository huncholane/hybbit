...
/**
 * A live transcript of an AI assistant working against the Hygo MCP
...
export function AgentConsole({ grid = true }: AgentConsoleProps = {}) {
  ...
  const scenarios: Scenario[] = [
    {
      ...
      tools: [
        { name: "get_overview_timeseries", args: "site: hygo.ai · past 7 days" },
  ...
  return (
    ...
    >
      <p className="sr-only">
        {t(
          "Example conversation: an AI assistant connected to Hygo over MCP pulls live analytics, investigates errors and funnels, and creates goals on request."
        )}
      </p>
      {grid && (
        <div
