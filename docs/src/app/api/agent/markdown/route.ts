...
function sourceOrigin(requestUrl: URL): string {
  ...
  const configured = validConfiguredOrigin(process.env.HYGO_SITE_ORIGIN);
  ...
  if (hostname === "hygo.ai" || hostname === "www.hygo.ai") return requestUrl.origin;
...
async function markdownRequest(request: Request): Promise<Response> {
  ...
  const pathname = request.headers.get("X-Hygo-Markdown-Path") || requestUrl.searchParams.get("path") || "/";
  const originalSearch = request.headers.get("X-Hygo-Markdown-Query") || requestUrl.searchParams.get("query") || "";
