...
describe("dimension normalisation", () => {
  ...
  it("maps Plausible's browser, os and device names onto Hygo's", async () => {
    const { events } = await runImport(
      single({
...
describe("custom events", () => {
  ...
  it("carries link_url through as a url prop", async () => {
    const { events } = await runImport({
      "pages.csv": BASIC_PAGES,
      "custom_events.csv":
        "date,name,visitors,events,path,link_url\n2024-03-01,Outbound Link: Click,1,1,/,https://x.com/hygo\n",
    });

    expect(events.filter(e => e.type === "custom_event")[0].props).toBe('{"url":"https://x.com/hygo"}');
  });
