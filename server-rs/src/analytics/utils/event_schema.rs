//! Port of server/src/api/analytics/utils/eventSchema.ts: the `scoped_events` column
//! reference given to the custom-query generator and the MCP tools.

/// `EVENT_SCHEMA`, byte for byte.
pub const EVENT_SCHEMA: &str = r#"
scoped_events columns:
- site_id UInt16: Numeric Hygo site id. scoped_events is already limited to sites the user can access.
...
- import_id Nullable(String): Import job id for rows loaded from imported analytics data; null for native Hygo tracking.
"#;
