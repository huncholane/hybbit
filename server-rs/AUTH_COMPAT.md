...
- `basePath` `/api/auth`; base URL from Better Auth's `getBaseURL` env chain (`BETTER_AUTH_URL`, `NEXT_PUBLIC_BETTER_AUTH_URL`, `PUBLIC_BETTER_AUTH_URL`, `NUXT_PUBLIC_BETTER_AUTH_URL`, `NUXT_PUBLIC_AUTH_URL`, then `BASE_URL` unless it is `/`), with `/api/auth` appended when the URL has no path; in production that is `${BASE_URL}/api/auth`. The Fastify `/auth/*` mount never matches Better Auth's router, so it is not ported.
...
- Captcha (Turnstile) only when `CLOUD=true`, `TURNSTILE_SECRET_KEY` set and production; a plugin `onRequest` that runs after the rate limiter and before routing, on any pathname that *contains* `/sign-up/email`, `/sign-in/email` or `/request-password-reset` (so `/email-otp/request-password-reset` too) but not `/sign-in/email-otp`. Missing `x-captcha-response` → 400 `{"message":"Missing CAPTCHA response","code":"MISSING_RESPONSE"}`; siteverify `success` false → 403 `VERIFICATION_FAILED` "Captcha verification failed"; siteverify unreachable, non-2xx or empty (10 s timeout) → 500 `UNKNOWN_ERROR` "Something went wrong". Bodies are bare `Response`s, so `content-type: text/plain;charset=UTF-8`. Ported (`captcha.rs`), verified by unit tests only (the parity Node runs without CLOUD).
- `dash()` (@better-auth/infra): its after hook is ported (non-GET requests with `X-Request-Id` get `__infra-rid=<id>; Max-Age=600; Path=/; HttpOnly; SameSite=Lax`, otherwise an existing `__infra-rid` cookie is expired). Its `/dash/*` and `/events/*` routes are not ported (Rust answers 404 where Node answers 401 `{"message":"Invalid API key"}` without an infra JWT). Note `POST /dash/check-user-exists` in Node only checks that an `Authorization` header is present (any value), then answers `{exists, userId}` for any email, which leaks account existence and user ids; that exposure exists in production today and could be closed by blocking `/api/auth/dash/*` at Caddy.
- CLOUD-only behaviour in auth.ts that the Rust port does not reproduce: the `invite-member` plan member limit (Stripe subscription lookup), the welcome email and Resend marketing contact on user creation, and the `/api-key/create` plan gate (the route itself is unported). Rust logs a warning once when `CLOUD=true`.
...
- `apikey.metadata` read path (probed with 15 stored shapes): drizzle `JSON.parse`s the jsonb text once, so the plugin starts from the jsonb value itself (JS key order: array-index keys first). The schema's `transform.output` maps falsy values to null and runs better-json `parseJSON` (strict) on strings: ISO date strings inside are revived (and print back with milliseconds), and a string that is not JSON (including a bare ISO timestamp) throws, which fails the whole `findMany`/`findOne` (list and delete answer 500 for that user). What Node itself writes (a jsonb string holding JSON text) therefore lists as an object. Only when that still yields a string does `batchMigrateLegacyMetadata` run: the response uses `safeJSONParse` of it (null on failure) and the row is rewritten to the jsonb value of `JSON.stringify(parsed)`, `updatedAt` untouched. `permissions` (text) goes through `safeJSONParse` (dates revived).
...
- Invitations: statuses `pending | accepted | rejected | canceled`; `expiresAt = now + 172800 s`; `teamId` holds comma-joined team ids; accepting needs no email verification. Limits 100 invitations, 100 members. Email link `${BASE_URL}/invitation?invitationId=${id}&organization=${org.name}&inviterEmail=${email}` (not URL-encoded). Hooks: `beforeCreateInvitation` validates site restrictions; after `accept-invitation`, copy `has_restricted_site_access`/`site_ids` onto the member and insert `member_site_access` rows; `afterRemoveMember`. The auth.ts after hook for `leave` never does anything in Node: the organization plugin's `shimContext` hands the endpoint a copy of the context, so the after hook never sees the session and skips the invitation purge and cache invalidation. Rust reproduces the no-op (verified differentially); fixing it would be a behaviour change.
...
- Authorize: not logged in → set `oidc_login_prompt`, redirect to `/login?<same query>`; any later response that sets `session_token` while that cookie exists resumes authorization (the stored `prompt` loses `login`; `none` combined with anything else is a 400 `{"message","error":"invalid_request","error_description":"prompt none must only be used alone"}` that still carries the new session cookie and the expired prompt cookie).
- `mcp/authorize` and `mcp/register` set `Access-Control-Allow-Origin: *` (plus methods `POST, OPTIONS`, headers `Content-Type, Authorization`, max-age 86400) on the response; in Node these win over @fastify/cors because the handler's headers are copied last, so a trusted Origin still sees `*` next to `Access-Control-Allow-Credentials: true`. `redirect_uri` must equal one of the comma-separated `redirectUrls`; scopes within `openid profile email offline_access` + custom. Code stored as a verification row `identifier = code`, `value = JSON{clientId, redirectURI, scope:[..], userId, authTime, requireConsent, state, codeChallenge, codeChallengeMethod:"s256", nonce}`, 600 s; redirect `redirect_uri?code=&state=`.
...
2. `customPlan` maps to a nonexistent `"customPlan"` column (the real one is `custom_plan`), so Better Auth reads return undefined and writes fail. The Rust auth endpoints match the responses: `customPlan` is never serialised.
...
7. `/organization/leave` cleanup in auth.ts never runs in Node (see Organizations). Rust keeps the no-op.
8. Better Auth's router answers an empty 404 for unknown paths, a wrong method, `HEAD`, a trailing slash and a doubled slash. Fastify gates bodies first: `text/plain` is swallowed (Better Auth sees no body), any other type, or a body without a content type, is a 415 `FST_ERR_CTP_INVALID_MEDIA_TYPE` rewritten by the api-errors hook.

## Rust port status (`src/auth/endpoints/`, `src/email/`)

Modules: `mod.rs` (router, pipeline, session resolution), `context.rs` (request context, `APIError`), `cookies.rs`, `crypto.rs` (ids, scrypt, HS256), `db.rs` (user, session, account, verification rows), `origin.rs` (trusted origins, CSRF), `rate_limit.rs`, `captcha.rs`, `validate.rs` (zod messages), `better_json.rs` (JSON.parse, better-json and safeJSONParse quirks), `core_routes.rs`, `email_otp.rs`, `organization.rs`, `api_key.rs`, `admin.rs`, `mcp.rs`, `dash.rs` (the `__infra-rid` after hook only). `src/email/` renders the OTP, invitation, verification-link and change-email mails byte for byte like react-email (fixtures in `src/email/fixtures/node-renders.json`) and sends through Resend only when `CLOUD=true`.

Mounting: `auth::endpoints::router()` answers every `/api/auth/*` path (ported routes in `ROUTES`, everything else Better Auth's empty 404); `auth::endpoints::mcp::well_known_router()` serves the six root discovery documents (`/.well-known/oauth-authorization-server[/api/mcp]`, `/.well-known/openid-configuration[/api/mcp]`, `/.well-known/oauth-protected-resource[/api/mcp]`). Both expect the HTTP edge layers (CORS, api error rewrite, request log) around them.

Not ported (`UNPORTED_ROUTES`, all 404 from Rust, none called by the dashboard): `/callback/:id`, `/reset-password`, `/reset-password/:token`, `/request-password-reset`, `/verify-password`, `/update-session`, `/link-social`, `/unlink-account`, `/list-accounts`, `/account-info`, `/refresh-token`, `/get-access-token`, `/delete-user/callback`, `/admin/create-user`, `/admin/remove-user`, `/admin/set-user-password`, `/api-key/create`, `/api-key/get`, `/api-key/update`, `/organization/list-members`, `/organization/get-active-member-role`, every team route, and every `/dash/*` and `/events/*` route. Social sign-in works up to the provider redirect; the callback stays on Node.

Verification: `parity/auth-endpoints/run.sh` starts Node (unmodified, TZ=UTC) and the Rust router behind the edge layers against the parity Postgres, runs every scenario once per backend with fresh `parity-auth-` fixtures, and compares responses (status, headers, Set-Cookie names, values after signature verification, attributes), normalised bodies and resulting rows step by step. The `sc_cross_*` scenarios run the round trips across backends in both directions: sessions, dont-remember sessions, refreshes and sign-out; password change and OTP reset then sign-in; impersonation then stop; OAuth code issued by one backend and exchanged by the other, access and refreshed tokens read by the other's `mcp/get-session`, Node's `POST /api/mcp` gate and Rust's `resolve_bearer_identity`. Last run: 489 of 503 steps identical. The differences:

- 12 steps: `Access-Control-Allow-Origin` on `mcp/register` and on the login-prompt resume of `mcp/authorize` is `*` in Node and the reflected origin in Rust, because `http::cors` overwrites the header the handler set. The CORS layer should keep an `Access-Control-Allow-Origin` the inner response already carries.
- 1 step: a cross-origin POST from an untrusted Origin is refused by `http::cors` with 403 `{"error":"Origin not allowed"}`, but Node's api-error rewrite turns it into `{"error","code":"FORBIDDEN","message","resolution"}` with `cache-control: no-store` and `x-content-type-options: nosniff`; `http::errors` skips `/api/auth/*`, which is right for Better Auth's own bodies but not for this Fastify-level rejection.
- 1 step: `/dash/*` is 404 in Rust and 401 `{"message":"Invalid API key"}` in Node (not ported, see Configuration).
