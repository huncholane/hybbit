...
- **Client in the Rust server** (user, 2026-09-17: "have the client served in the rust server... no need to run a second container for that"): the Next.js client becomes a static export baked into the `backend-rs` image; the Rust server serves it (route matching for dynamic segments, the `proxy.ts` redirects, the `/widget/:siteId` renderer) and the `client` container goes away. Live since 2026-09-17 16:01 UTC (Caddy catch-all to backend-rs; the client container was removed after the user confirmed the dashboard works).
...
- **Startup side effects:** all of a boot's database work is on Rust (`src/db_init/`, 2026-09-18), so a deployment with no Node container comes up on empty stores. In order, before serving: the drizzle migrations, then `initializeClickhouse` and `initPostgres` together. Details in "Startup database init" below.
...
Ported 2026-09-18 into `src/db_init/`, run from `main.rs` before the listener binds and before the ingest queues start. It replaces two separate pieces of the Node deployment: `server/docker-entrypoint.sh`'s `drizzle-kit migrate`, and `index.ts`'s `Promise.all([initializeClickhouse(), initPostgres()])`.

- **Migrations (`migrations.rs`)** reimplement `drizzle-orm/pg-core/dialect.js`'s `PgDialect.migrate` and `migrator.js`'s `readMigrationFiles`, which is what `drizzle-kit migrate` calls. Schema `drizzle`, table `__drizzle_migrations` (`id SERIAL PRIMARY KEY, hash text NOT NULL, created_at bigint`); `hash` is the SHA-256 of the whole `.sql` file; `created_at` is the journal's `when`; what runs is decided by the single newest `created_at`, and every pending file goes in one transaction. Hashes are never compared, so an edited applied file changes nothing, exactly as in drizzle. The 21 files and their journal entries are embedded with `include_str!` (like `server/package.json`'s version), and a test fails if `server/drizzle` and the embedded list disagree, so a new `npm run db:generate` file cannot be silently skipped. `server-rs/Dockerfile` copies `server/drizzle` into the build for that; the image needs no drizzle-kit and no migrate step.
- **ClickHouse (`clickhouse.rs`, `query_user.rs`)** is a verbatim port of `schema/{core,cloud,liteDashboard}.ts` and `queryUser.ts`, including the column-healing ALTERs, the refreshable `session_hourly_mv` replacement and the `hygo_query` settings profile, user and grant (`SqlString.escape` reproduced so a password with a quote or backslash produces Node's literal). The database itself is still made by the container's `CLICKHOUSE_DB`, as under Node.
- **Postgres (`postgres.rs`)** promotes the oldest user to admin on every boot, unconditionally, and creates the AppSumo tables when `CLOUD=true`. The AppSumo *routes* stay dropped; only the DDL is ported, and `CLOUD` is never set here.
- **Failure behaviour matches Node's, which is not uniform:** migrations fatal (`set -e` in the entrypoint), ClickHouse init fatal (`execClickhouseInitStep` rethrows and `start()` exits 1), `initPostgres` logged and survived, query-user provisioning logged and survived.
- **No cluster equivalent:** Node skips this in a worker because the primary ran it; one process per container means Rust always runs it.
- **Verified** by `parity/db-init/run.sh` (and `run.sh full` for the `CLOUD`/`LITE_DASHBOARD` branches): Node's boot against one throwaway Postgres+ClickHouse pair, Rust's against another, then a normalised diff of `pg_dump --schema-only`, `drizzle.__drizzle_migrations`, the `user` roles, `SHOW TABLES` + `SHOW CREATE TABLE` for every ClickHouse table and view, and the query user's `SHOW CREATE USER`/`SHOW GRANTS`/profile. Six checks: fresh parity, Rust twice, Rust over a Node-made store, admin promotion, identical healing of identical drift, and (with `PARITY_SNAPSHOT=backups/<timestamp>`) zero change against a restored production schema. All identical in both modes.
...
| POST | /api/session-replay/record/:siteId | none | api/sessionReplay/recordSessionReplay.ts | rust |
| GET | /api/site/tracking-config/:siteId | none | api/sites/getTrackingConfig.ts | rust |
| POST | /api/site/:siteId/feature-flags/evaluate | none | api/featureFlags/index.ts | rust |
...
| Method | Path | Guard | Node handler | Status |
|---|---|---|---|---|
| GET | …/session-replay/list | publicReplayRead | getSessionReplays.ts | rust |
| GET | …/session-replay/:sessionId | publicReplayRead | getSessionReplayEvents.ts | rust |
| DELETE | …/session-replay/:sessionId | authReplayWrite | deleteSessionReplay.ts | rust |
