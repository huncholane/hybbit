...
run_clickhouse_query() {
  ...
  printf '%s\n' "$query" | docker exec -i \
    -e HYGO_BACKUP_USER="${CLICKHOUSE_BACKUP_USER:-}" \
    -e HYGO_BACKUP_PASSWORD="${CLICKHOUSE_BACKUP_PASSWORD:-}" \
    -e HYGO_BACKUP_RECEIVE_TIMEOUT="$CLICKHOUSE_BACKUP_RECEIVE_TIMEOUT" \
    "$CLICKHOUSE_CONTAINER" sh -ec '
      if [ -n "$HYGO_BACKUP_USER" ]; then
        exec clickhouse-client --user "$HYGO_BACKUP_USER" --password "$HYGO_BACKUP_PASSWORD" --receive_timeout "$HYGO_BACKUP_RECEIVE_TIMEOUT" --multiquery
      fi
      exec clickhouse-client --user "$CLICKHOUSE_USER" --password "$CLICKHOUSE_PASSWORD" --receive_timeout "$HYGO_BACKUP_RECEIVE_TIMEOUT" --multiquery
    '
}
...
CLICKHOUSE_LOCK_FILE="${CLICKHOUSE_LOCK_FILE:-/run/lock/hygo-clickhouse-backup.lock}"
...
if [[ "$CLICKHOUSE_BACKUP_MODE" == "s3" ]]; then
  ...
  CLICKHOUSE_LOCAL_BACKUP_ROOT="${CLICKHOUSE_LOCAL_BACKUP_ROOT:-/var/backups/hygo/clickhouse}"
