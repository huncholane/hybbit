...
| Hygo | Native `pg_dump` to B2 | Not backed up here; production uses the standalone host |
...
```bash
sudo install -d -m 0755 /opt/hygo-backups/bin
sudo install -m 0755 ops/backups/bin/backup-postgres.sh /opt/hygo-backups/bin/
sudo install -m 0755 ops/backups/bin/backup-clickhouse.sh /opt/hygo-backups/bin/
sudo install -m 0644 ops/backups/bin/common.sh /opt/hygo-backups/bin/
```

Install the appropriate configuration without committing secrets:

```bash
# Choose frogstats.env.example, hygo.env.example, or clickhouse.env.example.
sudo install -m 0600 ops/backups/config/HOST.env.example /etc/hygo-backup.env
sudoedit /etc/hygo-backup.env
```
...
```bash
sudo env BACKUP_CONFIG=/etc/hygo-backup.env DRY_RUN=true \
  /opt/hygo-backups/bin/backup-postgres.sh
sudo env BACKUP_CONFIG=/etc/hygo-backup.env DRY_RUN=true \
  /opt/hygo-backups/bin/backup-clickhouse.sh
```

Only run the script relevant to the host. The Hygo application host does not
...
```bash
sudo install -d -m 0750 /etc/hygo-backup
sudo install -m 0640 -o root -g 101 \
  ops/backups/clickhouse/s3-named-collection.xml.example \
  /etc/hygo-backup/clickhouse-s3.xml
sudoedit /etc/hygo-backup/clickhouse-s3.xml
```
...
sudo install -m 0640 -o root -g 101 \
  ...
  /etc/hygo-backup/clickhouse-backup-user.xml
sudo sed -i \
...
services:
  clickhouse:
    volumes:
      - /etc/hygo-backup/clickhouse-s3.xml:/etc/clickhouse-server/config.d/backup-s3.xml:ro
      - /etc/hygo-backup/clickhouse-backup-user.xml:/etc/clickhouse-server/users.d/backup-user.xml:ro
```
...
sudo install -m 0640 -o root -g 101 \
  ...
  /etc/hygo-backup/clickhouse-backup-disk.xml
sudo install -d -m 0750 -o 101 -g 101 /var/backups/hygo/clickhouse
```
...
services:
  clickhouse:
    volumes:
      - /var/backups/hygo/clickhouse:/var/lib/clickhouse/backups
      - /etc/hygo-backup/clickhouse-backup-disk.xml:/etc/clickhouse-server/config.d/backup-disk.xml:ro
```
...
sudo systemctl start hygo-postgres-backup.service
sudo journalctl -u hygo-postgres-backup.service -n 100 --no-pager

# FrogStats and the standalone ClickHouse host only:
sudo systemctl start hygo-clickhouse-backup.service
sudo journalctl -u hygo-clickhouse-backup.service -n 100 --no-pager
```

After verifying objects at every configured destination:

```bash
sudo systemctl enable --now hygo-postgres-backup.timer
# FrogStats and the standalone ClickHouse host only:
sudo systemctl enable --now hygo-clickhouse-backup.timer
```
